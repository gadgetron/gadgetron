...
def get_meta_dependencies(path_to_meta: str):
    ...
    if 'darwin' in sys.platform.lower():
        excluded_platforms = ['linux64']
    else:
        excluded_platforms = ['osx']

    with open(path_to_meta) as file:
        for i in range(7):
            _ = file.readline() # Seek past the version directive as its not valid YML.

        with open ('temp_meta.yml', 'w') as f:
            for line in file:
                if any (excluded_platform in line for excluded_platform in excluded_platforms):
                   pass  # don't include lines with excluded platform flags
                else:
                   f.write("%s" % line)

        with open('temp_meta.yml', 'r') as filtered_file:
            file_contents = yaml.safe_load(filtered_file)
...
def main():
    ...
    if 'darwin' in sys.platform.lower():
       environment_path = os.path.abspath(os.path.join(current_dir, '../environment-macos.yml'))
    else:
    ...
    for meta_pkg_name, meta_pkg_dep in meta_deps.items():
        if not environment_deps.get(meta_pkg_name):
            raise RuntimeError(f'{meta_pkg_name} defined in meta.yaml not found in {environment_path}')
        else:
            ...
            if meta_pkg_dep != env_dep:
                ...
                if version.parse(meta_pkg_dep.version.removesuffix('.*')) < version.parse(env_dep.version.removesuffix('.*')):
                    errors.append(f'{env_dep!s} defined in {environment_path} is newer than {meta_pkg_dep!s} defined in meta.yaml')
