...
if [[ $(uname) =~ Darwin ]]; then
   ...
   export SYSTEM_DIRECTORIES_TO_IGNORE='Applications/Xcode.app_14.0.1.app/Contents/Developer/usr/bin;/Applications/Xcode_14.0.1.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX12.3.sdk/usr/lib;/usr/bin;/usr/lib;/usr/local/bin;/usr/local/lib'
   cmake -GNinja -DCMAKE_BUILD_TYPE=Release -DCMAKE_IGNORE_PATH=$SYSTEM_DIRECTORIES_TO_IGNORE -DCMAKE_C_COMPILER=${PREFIX}/bin/clang -DCMAKE_CXX_COMPILER=${PREFIX}/bin/clang++ -DBUILD_PYTHON_SUPPORT=ON -DUSE_MKL=ON -DUSE_CUDA=OFF -DCMAKE_INSTALL_PREFIX="${PREFIX}" "${SRC_DIR}"
else
