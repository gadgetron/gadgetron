...
if [[ $(uname) =~ Darwin ]]; then
    ...
    cmake -GNinja -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=${CONDA_PREFIX}/bin/clang -DCMAKE_CXX_COMPILER=${CONDA_PREFIX}/bin/clang++ -DUSE_MKL=ON -DUSE_CUDA=OFF -DBUILD_PYTHON_SUPPORT=OFF -DCMAKE_INSTALL_PREFIX="${PREFIX}" "${SRC_DIR}"
else
