...
if [[ $(uname) =~ Darwin ]]; then
    ...
    cmake -GNinja -DCMAKE_BUILD_TYPE=Release -DBUILD_PYTHON_SUPPORT=OFF -DCUDA_COMPUTE_CAPABILITY=ALL -DUSE_MKL=ON -DUSE_CUDA=ON -DCMAKE_INSTALL_PREFIX="${PREFIX}" "${SRC_DIR}"
fi
ninja && ninja install
