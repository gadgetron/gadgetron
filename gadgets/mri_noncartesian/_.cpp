...
namespace Gadgetron {
	...
	{
		std::cout << process_called_times_ << std::endl;
		if (perform_timing.value()) { gt_timer_local_.start("CPUGriddingReconGadget::process"); }
		...
		for (size_t e = 0; e < recon_bit_->rbit_.size(); e++)
		{
			...
			if (E2 > 1) {
				GERROR("3D data is not supported in CPUGriddingReconGadget\n");
				m1->release();
				return GADGET_FAIL;
			}
			...
			imarray.data_.create(image_dims_os_[0],image_dims_os_[1], 1, 1, N, S, SLC);
...
// 			}
	...
	boost::shared_ptr<cuNDArray<float_complext> > CPUGriddingReconGadget::reconstruct(
		...
		if (!iterate.value() && dcw) { 
			...
			std::vector<size_t> recon_dims = {image_dims_os_[0], image_dims_os_[1]};
			...
			hoNDArray<float_complext> datah(data->get_dimensions());
			hoNDArray<floatd2> trajh(traj->get_dimensions());
			hoNDArray<floatd2> flat_traj(traj->get_dimensions());
			hoNDArray<float> dcwh(dcw->get_dimensions());

			std::copy(data->begin(), data->end(), datah.begin());
			std::copy(traj->begin(), traj->end(), trajh.begin());
			std::copy(dcw->begin(), dcw->end(), dcwh.begin());
			std::cout << "-----------" << std::endl;
			std::cout << "Data elements: " << datah.get_number_of_elements() << std::endl;
			std::cout << "Traj elements: " << trajh.get_number_of_elements() << std::endl;
			std::cout << "Dcw elements: " << dcwh.get_number_of_elements() << std::endl;
			for(auto it = datah.end()-5; it != datah.end(); it++)
				std::cout << *it << ", ";
			std::cout << std::endl;
			
			plan.preprocess(&trajh,hoNFFT_plan<float,2>::NFFT_PREP_NC2C);
			plan.compute(&datah,result,&dcwh,hoNFFT_plan<float,2>::NFFT_BACKWARDS_NC2C);
			auto a = new cuNDArray<float_complext>(recon_dims);
			std::copy(result->begin(), result->end(), a->begin());
			return boost::shared_ptr<cuNDArray<float_complext>>(a);
	...
	std::tuple<boost::shared_ptr<hoNDArray<floatd2 > >, boost::shared_ptr<hoNDArray<float >>> CPUGriddingReconGadget::separate_traj_and_dcw(
