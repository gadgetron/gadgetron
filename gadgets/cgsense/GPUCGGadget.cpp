...
int GPUCGGadget::process_config(ACE_Message_Block* mb)
{
  ...
  if (!is_configured_) {
	  ...
	  // Code that used to belong in set_base_parameters in the old days
	  /*
	  ...
	  */
	  ...
	  matrix_size_os_ = 
		  make_uintd2(static_cast<unsigned int>(ceil((matrix_size_.x*oversampling_)/32.0f)*32),  //TODO: use warp_size from deviceProp over 32 - it might change some day
		              static_cast<unsigned int>(ceil((matrix_size_.y*oversampling_)/32.0f)*32)); //TODO: use warp_size from deviceProp over 32 - it might change some day
	  ...
	  D_ = boost::shared_ptr< cuCGPrecondWeight<float_complext::Type> >( new cuCGPrecondWeight<float_complext::Type>() );
	  ...
	  cg_.set_output_mode( cuCG<float, float_complext::Type>::OUTPUT_VERBOSE ); // TODO: once it is all working, change to silent output
	  ...
	  std::vector<unsigned int> csm_dims = uintd_to_vector<2>(matrix_size); csm_dims.push_back( channels_ );
	  csm_->create( &csm_dims );
	  cuNDA_clear( csm_->get(), 1.0f );
...
//    int samples_in_frame = profiles_per_frame_*samples_per_profile_;
    ...
    if( E->preprocess(traj.get()) < 0 ) {
      GADGET_DEBUG1("Error during NFFT preprocessing\n");
      return GADGET_FAIL;
    }
 ...
 */   
    ...
    */
    ...
    E_->mult_MH( device_samples.get(), &rhs );
