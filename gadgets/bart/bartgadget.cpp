...
namespace Gadgetron {
	...
	std::string &Bartgadget::getCommandsScript(const std::string &ConfigFile)
	{
		namespace pt = boost::property_tree;
		pt::ptree tree;
		pt::read_xml(ConfigFile, tree);
		static std::string filename;
		BOOST_FOREACH(pt::ptree::value_type &v, tree.get_child("gadgetronStreamConfiguration")) {
			if (v.first == "gadget") {
				BOOST_FOREACH(pt::ptree::value_type &w, v.second) {
					if (w.first == "BartCommandsFile") {
						filename = std::string(w.second.data());
					}
				}
			}
		}
	...
	int Bartgadget::process(GadgetContainerMessage<IsmrmrdReconData>* m1)
	{
		...
		{
			GERROR("Error getCommandsScript: %s\n", e.what());
		}
