...
if __name__ == "__main__":
    ...
    ch2_ch3_ch4_model_onnx_file = 'CMR_landmark_network_RO_320_E1_320_CH2_CH3_CH4_Myo_Pts_sFOV_LossMultiSoftProb_KLD_Dice_Pytorch_1.8.0a0+37c1f4a_2021-08-08_20210808_085042.onnx'
    model_onnx_ch2_ch3_ch4 = load_model_onnx(model_dir, ch2_ch3_ch4_model_onnx_file)
    print(model_onnx_ch2_ch3_ch4)

    print("=================================================================")
    print("Test RetroCine, CH4, LV, with heart centering, for all phases")

    data_file = os.path.join(GT_CMR_ML_UT_HOME, 'data', 'cmr_landmark_detection', 'RTCine', 'ch4_RT_Cine_LIN_66016_1056888010_1056888019_340_20200131-141649', 'data.npy')
    print("load - ", data_file)
    im = np.load(data_file)
    print(im.shape)

    res_dir = os.path.join(GT_CMR_ML_UT_HOME, 'result', 'cmr_landmark_detection', 'RTCine', 'ch4_RT_Cine_LIN_66016_1056888010_1056888019_340_20200131-141649')
    os.makedirs(res_dir, exist_ok=True)

    pts, probs, im_used = perform_cmr_landmark_detection(im, model_onnx_ch2_ch3_ch4, p_thresh=0.1, oper_RO=320, oper_E1=320)
    
    print('PTs ', pts.shape)
    print('Probs ', probs.shape)

    res_file = os.path.join(res_dir, 'im.npy')
    print("save - ", res_file)
    np.save(res_file, im)

    res_file = os.path.join(res_dir, 'pts.npy')
    print("save - ", res_file)
    np.save(res_file, pts)

    res_file = os.path.join(res_dir, 'probs.npy')
    print("save - ", res_file)
    np.save(res_file, probs)

    res_file = os.path.join(res_dir, 'im_used.npy')
    print("save - ", res_file)
    np.save(res_file, im_used)
