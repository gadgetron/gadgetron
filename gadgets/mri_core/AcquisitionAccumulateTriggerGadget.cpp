...
namespace Gadgetron {
    ...
    ::process(GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *m1) {
        ...
        GadgetContainerMessage<hoNDArray<std::complex<float> > > *m2 = AsContainerMessage<hoNDArray<std::complex<float> > >(
                m1->cont());
        if (!m2) {
        ...
        if (ISMRMRD::FlagBit(ISMRMRD::ISMRMRD_ACQ_IS_PARALLEL_CALIBRATION).isSet(m1->getObjectPtr()->flags) ||
            ISMRMRD::FlagBit(ISMRMRD::ISMRMRD_ACQ_IS_PARALLEL_CALIBRATION_AND_IMAGING).isSet(
                    m1->getObjectPtr()->flags)) {
