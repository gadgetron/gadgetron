...
process(ACE_Message_Block * mb)
{
	GadgetContainerMessage<ISMRMRD::AcquisitionHeader>* m1 = AsContainerMessage<ISMRMRD::AcquisitionHeader>(mb);

	GadgetContainerMessage<hoNDArray<std::complex<float> > >* m2 = 0;
	if (m1) {
		m2 = AsContainerMessage<hoNDArray<std::complex<float> > >(m1->cont());
	}

	GadgetContainerMessage<hoNDArray<float> >* m3 = 0;
	bool has_trajectory = m1->getObjectPtr()->trajectory_dimensions > 0;
	if (has_trajectory){
		if (m2) {
			m3 = AsContainerMessage<hoNDArray<float> >(m2->cont());
		}
	}

	if (!m1 || !m2 || (!m3 && has_trajectory)) {
		if (!pass_on_undesired_data_) {
			ACE_DEBUG( (LM_ERROR, ACE_TEXT("%s -> %s, (%s, %s, %@), (%s, %s, %@), (%s, %s, %@)\n"),
					...
					typeid(GadgetContainerMessage<ISMRMRD::AcquisitionHeader>*).name(),
					typeid(m1).name(),
					m1,
					typeid(GadgetContainerMessage<hoNDArray<std::complex<float> > >*).name(),
					typeid(m2).name(),
					m2,
					typeid(GadgetContainerMessage<hoNDArray<float> >*).name(),
					typeid(m3).name(),
					m3));
	...
	if (!buffer_) {
		...
		try {
			buffer_ = boost::shared_ptr<hoNDArray<std::complex<float> > >(
					new hoNDArray<std::complex<float> >(&dimensions_));
		} catch (std::runtime_error &err) {
			GADGET_DEBUG1("Failed to create buffer\n");
			return GADGET_FAIL;
		}
