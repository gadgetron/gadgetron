...
namespace Gadgetron{
    ...
    int CoilReductionGadget::process_config(ACE_Message_Block *mb)
    {
        ...
        if (coil_mask->compare(std::string("")) == 0) {
            ...
            for (unsigned long long i = 0; i < chm.size(); i++) {
                std::string ch = boost::algorithm::trim_copy(chm[i]);
                if (ch.size() > 0) {
                    unsigned long long mv = static_cast<unsigned long long>(ACE_OS::atoi(ch.c_str()));
        ...
        coils_out_ = 0;
        for (unsigned long long i = 0; i < coil_mask_.size(); i++) {
    ...
    {
        std::vector<unsigned long long> dims_out(2);
        ...
        unsigned long long coils_copied = 0;
