    ...
    int PCACoilGadget::process(GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *m1, GadgetContainerMessage<hoNDArray<std::complex<float> > > *m2)
    {
        ...
        if (is_buffering) {
            ...
            if (is_last_scan_in_slice || (profiles_available >= max_buffered_profiles_)) {
                ...
                std::vector<unsigned long long> dims(2);
                ...
                unsigned long long sample_counter = 0;
                ...
                std::vector<unsigned long long> means_dims; means_dims.push_back(channels);
                ...
                for (unsigned long long p = 0; p < profiles_available; p++) {
                ...
                //Subtract off mean
                for (unsigned long long c = 0; c < channels; c++) {
                    for (unsigned long long s = 0; s < total_samples; s++) {
                ...
                std::vector<unsigned long long> VT_dims;
