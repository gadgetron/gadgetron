...
template <class T> int GrappaWeightsCalculator<T>::svc(void)  {
	...
	while (this->getq(mb) >= 0) {
		...
		std::vector<unsigned long long> ftdims(2,0); ftdims[1] = 1;
		cuNDFFT<float> ft;
		...
		boost::shared_ptr< std::vector<unsigned long long> > data_dimensions = device_data.get_dimensions();
		...
		if (mb1->getObjectPtr()->destination) {
			...
			boost::shared_ptr< std::vector<unsigned long long> > tmp_dims = mb2->getObjectPtr()->get_dimensions();
			if (uncombined_channels_.size()) tmp_dims->push_back( (unsigned long long) (uncombined_channels_.size()+1) );
