...
namespace Gadgetron {
    ...
    template<class T>
    class EXPORTGADGETSGRAPPA GrappaWeightsDescription {
    ...
    int GrappaWeightsCalculator<T>::svc(void) {
        ...
        while (this->getq(mb) >= 0) {
            ...
            if (use_gpu_) {
                ...
                if (numUnCombined == 0) {
                    hoNDArray<std::complex<float> > acs(RO, E1, target_coils_,
                                                        reinterpret_cast< std::complex<float> * >(host_data->begin()));
                    ...
                    if (mb1->getObjectPtr()->acceleration_factor == 1) {
                        ...
                        ind = 1;
                        for (it = uncombined_channels_.begin(); it != uncombined_channels_.end(); it++) {
                            memcpy(unmixing_.begin() + ind * RO * E1 * CHA, kIm_.begin() +
                                                                            (target_coils_with_uncombined -
                                                                             numUnCombined + ind - 1) * RO * E1 * CHA,
                ...
                if (mb1->getObjectPtr()->destination) {
                    boost::shared_ptr<hoNDArray<std::complex<float> > > unmixing_host(
                            new hoNDArray<std::complex<float> >());
                    boost::shared_ptr<std::vector<size_t> > tmp_dims = mb2->getObjectPtr()->get_dimensions();
                    if (uncombined_channels_.size()) tmp_dims->push_back((size_t) (uncombined_channels_.size() + 1));
                    ...
                    catch (std::runtime_error &err) { GEXCEPTION(err, "Reshaping of GRAPPA weights failed \n");
    ...
    template<class T>
    int GrappaWeightsCalculator<T>::close(unsigned long flags) {
        int rval = 0;
        if (flags == 1) {
    ...
    template<class T>
    int GrappaWeightsCalculator<T>::add_uncombined_channel(unsigned int channel_id) {
    ...
    template<class T>
    int GrappaWeightsCalculator<T>::remove_uncombined_channel(unsigned int channel_id) {
        uncombined_channels_.remove(channel_id);
        return 0;
    }

    template
    class EXPORTGADGETSGRAPPA GrappaWeightsDescription<float>;

    template
    class EXPORTGADGETSGRAPPA GrappaWeightsCalculator<float>;
}
