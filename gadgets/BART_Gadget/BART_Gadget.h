...
#if defined (WIN32)
#ifdef __BUILD_GADGETRON_BART_Gadget__
#define EXPORTGADGETS_BART_Gadget __declspec(dllexport)
#else
#define EXPORTGADGETS_BART_Gadget __declspec(dllimport)
#endif
#else
#define EXPORTGADGETS_BART_Gadget
#endif


namespace Gadgetron {

	class EXPORTGADGETS_BART_Gadget BART_Gadget : public Gadget1<IsmrmrdReconData>

	{

	public:
		GADGET_DECLARE(BART_Gadget);
	...
	template<typename T>
	void BART_Gadget::write_BART_hdr(const char* filename, std::vector<T> &DIMS)
	{
		...
		if (!pFile.is_open())
			std::cerr << "Failed to write into file: " << filename << std::endl;
		pFile << "# Dimensions\n";
	...
	template<typename T, typename U>
	void BART_Gadget::write_BART_Files(const char* filename, std::vector<T> &DIMS, std::vector<U> &DATA)
	{
