...
namespace Gadgetron {
	...
	std::vector<size_t> BART_Gadget::read_BART_hdr(const char *filename)
	{
		std::string filename_s = std::string(filename) + std::string(".hdr");
		std::ifstream infile(filename_s);
		if (!infile.is_open())
			std::cerr << "Failed to open file: " << filename_s << std::endl;
	...
	std::pair< std::vector<size_t>, std::vector<std::complex<float> > >
		BART_Gadget::read_BART_files(const char *filename)
	{
	...
	int BART_Gadget::process(GadgetContainerMessage<IsmrmrdReconData>* m1)
	{
		...
		for (std::vector<IsmrmrdReconBit>::iterator it = m1->getObjectPtr()->rbit_.begin(); it != m1->getObjectPtr()->rbit_.end(); ++it)
		{
			...
			// Setup data to be written into files
			...
			/* The reference data will be pointing to the image data if there is
			    ...
			    into files if it's pointing to the image data.*/
			if (DIMS_ref != DIMS) {
			...
			write_BART_Files("meas_gadgetron", DIMS, Temp);
		}
		...
		std::string filename("BART_Commands_.txt");
		...
		if (inputFile.is_open())
		{
			while (getline(inputFile, Commands_Line))
			{
				if (Commands_Line.empty())
					continue;
				std::cout << Commands_Line << std::endl;
				system(Commands_Line.c_str());
			}
			inputFile.close();
		}
		else
		...
		std::vector<std::string> outputFile, outputfile_2;
		boost::char_separator<char> sep(" ");
		boost::tokenizer<boost::char_separator<char> > tokens(Commands_Line, sep);
		for (auto itr = tokens.begin(); itr != tokens.end(); ++itr)
			outputFile.push_back(*itr);

		// Reformat the data back to gadgetron format
		auto header = read_BART_hdr(outputFile.back().c_str());
		cmd4 << "bart reshape 1023 " << header[1] << " " << header[2] << " " << header[0] << " " << header[3] << " " << header[9] * header[4]
			<< " 1 1 1 1 1 " << outputFile.back() << " " << outputFile.back() + std::string("_reshape");
		...
		boost::tokenizer<boost::char_separator<char> > tok(cmd_s, sep);
		for (auto itr = tok.begin(); itr != tok.end(); ++itr)
			outputfile_2.push_back(*itr);

		/**** READ FROM BART FILES ***/
		std::pair< std::vector<size_t>, std::vector<std::complex<float> > > BART_DATA = read_BART_files(outputfile_2.back().c_str());
	...
	GADGET_FACTORY_DECLARE(BART_Gadget)
}
