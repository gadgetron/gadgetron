...
namespace Gadgetron{
	  ...
	  GadgetContainerMessage< hoNDArray< std::complex<float> > > *m2)
  {
	...
	for (int i = 0; i < (samples_per_interleave_*Nints_); i++) {
      ...
      nfft_plan_.setup( from_std_vector<unsigned long long,2>(image_dimensions_recon_), image_dimensions_recon_os_, kernel_width_ );
      ...
      E_->setup( from_std_vector<unsigned long long,2>(image_dimensions_recon_), image_dimensions_recon_os_, kernel_width_ );
    ...
    if (!host_data_buffer_.get()) {
      ...
      std::vector<unsigned long long> data_dimensions;
	...
	if( buffer_using_solver_ ){
	  ...
	  std::vector<unsigned long long> domain_dims = image_dimensions_recon_;
	  
	  std::vector<unsigned long long> codomain_dims = *host_traj_->get_dimensions();
	...
	*(header->getObjectPtr()) = tmp;
	...
	std::vector<unsigned long long> image_dims;
	...
	std::vector<unsigned long long> ddimensions;
