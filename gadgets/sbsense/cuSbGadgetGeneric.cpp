...
namespace Gadgetron{
  ...
  cuSbGadgetGeneric::cuSbGadgetGeneric()
  : channels_(0)
  , device_number_(0)
  ...
  int cuSbGadgetGeneric::process_config( ACE_Message_Block* mb )
  {
    GADGET_DEBUG1("cuSbGadgetGeneric::process_config\n");
  ...
  int cuSbGadgetGeneric::process(GadgetContainerMessage<ISMRMRD::ImageHeader>* m1, GadgetContainerMessage<SenseJob> * m2)
  {
    GADGET_DEBUG1("cuSbGadgetGeneric::process\n");

    GPUTimer timer("cuSbGadgetGeneric::process");
    ...
    if (!j->csm_host_.get() || !j->dat_host_.get() || !j->tra_host_.get() || !j->dcw_host_.get()) {
      GADGET_DEBUG1("Received an incomplete SBSense JOB\n");
      m1->release();
      return GADGET_FAIL;
    }
  ...
  GADGET_FACTORY_DECLARE(cuSbGadgetGeneric)
}
