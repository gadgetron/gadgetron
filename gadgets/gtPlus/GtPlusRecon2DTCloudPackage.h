...
template <typename T> 
bool GtPlusRecon2DTCloudPackage<T>::serialize(char*& buf, unsigned long long& len) const
{
    ...
    try
    {
        ...
        // find the total len
        unsigned long long lenKSpace, lenRef, lenComplexIm, lenRes;
...
template <typename T> 
bool GtPlusRecon2DTCloudPackage<T>::deserialize(char* buf, unsigned long long& len)
{
    try
    {
        memcpy(&para, buf, sizeof(GtPlusRecon2DTPara));

        unsigned long long offset(sizeof(GtPlusRecon2DTPara)), currLen=0;
