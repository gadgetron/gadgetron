...
namespace Gadgetron
{
    ...
    public:
        virtual ACE_Message_Block* read(ACE_SOCK_Stream* stream) 
        {
            ...
            int id = 0;
            unsigned long long sizeOfJob = 0;
            ...
            *(jobID->getObjectPtr()) = id;
            ...
            if ((recv_count = stream->recv_n(&sizeOfJob, sizeof(unsigned long long))) <= 0)
            {
            ...
            unsigned long long maxBytesPerSend = 512.0*1024*1024;

            if ( sizeOfJob > maxBytesPerSend )
            {
                unsigned long long receivedBytes = 0;
                unsigned long long receivingBytes = maxBytesPerSend;
    ...
    public:
        ...
        virtual int write(ACE_SOCK_Stream* sock, ACE_Message_Block* mb) 
        {
            ...
            if ((send_cnt = sock->send_n (&sizeOfJob, sizeof(unsigned long long))) <= 0)
            {
                ACE_DEBUG ((LM_ERROR,
            ...
            if ( sizeOfJob > maxBytesPerSend )
            {
                ...
                unsigned long long sendingBytes = maxBytesPerSend;
