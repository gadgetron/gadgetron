...
namespace Gadgetron{
  ...
  int NFFT2DGadget::process_config(ACE_Message_Block* mb)
  {
    ...
    if (e_seq.size() != 1) {
      ...
      GADGET_DEBUG1("This simple NFFT2DGadget only supports one encoding space\n");
      return GADGET_FAIL;
    }
  ...
  {    
    ...
    if (is_last_scan_in_repetition) {
      ...
      extract_trajectory_and_dcw_from_queue( frame_traj_queue_.get(), &traj, &dcw );
      ...
      plan.compute( &samples, &image,  
                    (dcw.get_number_of_elements()>0) ? &dcw : 0x0, 
