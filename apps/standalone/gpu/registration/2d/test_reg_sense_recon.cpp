...
template<class REAL, unsigned long long D> class registrationReconOperator
...
public:
  ...
  virtual void mult_M( cuNDArray< complext<REAL> > *in, cuNDArray< complext<REAL> > *out, bool accumulate = false)
  {
    ...
    std::vector<unsigned long long> tmp_dims = *R_->get_displacement_field()->get_dimensions(); tmp_dims.pop_back();
  ...
  virtual void mult_MH( cuNDArray< complext<REAL> > *in, cuNDArray< complext<REAL> > *out, bool accumulate = false )
  {
    ...
    // Allocate intermediate image
    std::vector<unsigned long long> tmp_dims = *R_->get_displacement_field()->get_dimensions().get(); tmp_dims.pop_back();
...
int main(int argc, char** argv)
{
  ...
  std::vector<unsigned long long> tmp_vec = to_std_vector(matrix_size);
  ...
  *_precon_weights += *R_diag;
  ...
  for( unsigned int reconstruction = 0; reconstruction<num_reconstructions; reconstruction++ ){
    ...
    vector<unsigned long long> rhs_dims = to_std_vector(matrix_size); 
...
#ifdef PAD_Z
  std::vector<unsigned long long> _3d_dims = *(sense_result->get_dimensions());
  ...
  vector<unsigned long long> multi_dims = *sense_result->get_dimensions();
  multi_dims.pop_back();
#ifdef PAD_Z
...
#endif
  vector<unsigned long long> single_dims = *sense_result->get_dimensions();
  ...
  std::vector<unsigned long long> rhs_dims = to_std_vector(matrix_size); 
