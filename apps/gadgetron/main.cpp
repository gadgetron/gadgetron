...
void print_usage()
{
    ...
    ACE_DEBUG((LM_INFO, ACE_TEXT("            -c <WORKINGDIRECTORY>          (default '/tmp/gadgetron' in linux and 'c:\\temp\\gadgetron' in windows)       \n") ));
}
...
int ACE_TMAIN(int argc, ACE_TCHAR *argv[])
{
    ...
    #ifdef _WIN32
        workingdirectory = "c:\\temp\\gadgetron";
    #else
        workingdirectory = "/tmp/gadgetron";
    #endif // _WIN32
    ...
    try
    {
        ...
        if ( cfg->workingdirectory().present() )
        {
            workingdirectory = cfg->workingdirectory().get();
        }
    ...
    static const ACE_TCHAR options[] = ACE_TEXT(":p:c:");
    ...
    while ((option = cmd_opts()) != EOF) {
        ...
        case 'p':
            ...
            break;
        case 'c':
            workingdirectory = std::string(cmd_opts.opt_arg());
            break;
        case ':':
            print_usage();
    ...
    acceptor.working_directory_ = workingdirectory;
