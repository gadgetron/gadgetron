...
protected:
	int process(ACE_Message_Block* mb)
	{
		...
		if (!m1 || !m2 || !m3) {
			if (!pass_on_undesired_data_) {
				ACE_Message_Block* problem;
				std::string expected;
				if (m1){
					if ( m2){
						problem = m2->cont();
						expected =typeid(GadgetContainerMessage<P3>).name();
					} else {
						problem = m1->cont();
						expected =typeid(GadgetContainerMessage<P2>).name();
					}
				} else{
					problem = mb;
					expected =typeid(GadgetContainerMessage<P1>).name();
				}


				ACE_DEBUG( (LM_ERROR, ACE_TEXT("%s -> %s, Expected type (%s) but received (%s) from pointer (%@)\n"),
						this->module()->name(),
						ACE_TEXT("Gadget3::process, Conversion of Message Block Failed"),
						expected.c_str(),typeid(*problem).name()));
