...
namespace Gadgetron::Server::Distributed {
    ...
    RemoteChannel::RemoteChannel(const Address& address, const std::string& xml_config,
    ...
    void RemoteChannel::handle_close() {
        std::lock_guard guard(closed_mutex);
        closed_input  = true;
        closed_output = true;
    ...
    void RemoteChannel::save_error(const std::string& error_message) {
