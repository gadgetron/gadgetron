...
def stream_data_to_gadgetron(echo_handler, port, storage_address, *, input, output, configurations, input_adapter, output_adapter, output_group, log_stdout, log_stderr):
    stream_command = f"{input_adapter} -i {input}"

    commands = [f'gadgetron -p {port} -E {storage_address} --from_stream -c {configuration}' for configuration in configurations]
