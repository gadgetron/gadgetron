...
/*  -- MAGMA (version 1.0) --
    ...
    Purpose   
    =======   
    DGEHRD reduces a COMPLEX general matrix A to upper Hessenberg form H by   
	...
	*info = -8;
    ...
    if (nb < nbmin || nb >= nh) {
      ...
      cublasSetMatrix(N, N, sizeof(cuFloatComplex), a+(ilo-1)*(lda),lda, d_A, ldda);
	...
	cublasGetMatrix(ihi-i__+ilo, ib, sizeof(cuFloatComplex), 
