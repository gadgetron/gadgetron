...
/*  -- MAGMA (version 1.0) --
	...
	*info = -9;
    ...
    if (*info == 0) {
      ...
      ldwork = lddwork = n;
      lwkopt = n * nb;
      MAGMA_S_SET2REAL( work[1], lwkopt );
    }
	...
	for (i__ = n - nb + 1; i__ >= i__1; i__ -= nb) 
	  {
	    ...
	    i__3 = i__ - 1;
	    blasf77_ssyr2k(uplo_, "No transpose", &i__3, &nb, &z_neg_one, 
	    ...
	    i__3 = i__ + nb - 1;
	    for (j = i__; j <= i__3; ++j) {
		MAGMA_S_SET2REAL( a[j - 1 + j * a_dim1], e[j - 1] );
		d[j] = MAGMA_S_GET_X( a[j + j * a_dim1] );
	    }
	  }

	/*  Use unblocked code to reduce the last or only block */
	lapackf77_ssytd2(uplo_, &kk, &a[a_offset], &lda, &d[1], &e[1], &tau[1], &iinfo);
    } 
    else 
      {
	/* Copy the matrix to the GPU */
	...
	for (i__ = 1; i__ <= n-nx; i__ += nb) 
	  {
	    ...
	    cublasSsyr2k('L', 'N', i__3, nb, z_neg_one, 
	    ...
	    i__3 = i__ + nb - 1;
	    for (j = i__; j <= i__3; ++j) {
		MAGMA_S_SET2REAL( a[j + 1 + j * a_dim1], e[j] );
		d[j] = MAGMA_S_GET_X( a[j + j * a_dim1] );
	    }
	  }
	...
	lapackf77_ssytrd(uplo_, &i__1, &a[i__ + i__ * a_dim1], &lda, &d[i__], &e[i__],
