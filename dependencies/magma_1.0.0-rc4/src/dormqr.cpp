...
/*  -- MAGMA (version 1.0) --
    ...
    cublasSetMatrix( m, n, sizeof(double), c, ldc, dc, ldc);
    dc -= (1 + m);
    //-------------------------------------------------------------------------
	...
	*info = -12;
    ...
    if (nb < nbmin || nb >= k) 
      {
	/* Use unblocked code */
	lapackf77_dorm2r(side_, trans_, &m, &n, &k, &a[a_offset], &lda, &tau[1], 
	...
	for (i__ = i1; i3 < 0 ? i__ >= i2 : i__ <= i2; i__ += i3) 
	  {
	    ...
	    i__4 = nq - i__ + 1;
	    lapackf77_dlarft("F", "C", &i__4, &ib, &a[i__ + i__ * a_dim1], &lda, 
		    &tau[i__], t, &ib);

	    // TTT ------------------------------------------------------------
	    dpanel_to_q('U', ib, &a[i__ + i__ * a_dim1], lda, t+ib*ib);
	    cublasSetMatrix(i__4, ib, sizeof(double),
			    &a[i__ + i__ * a_dim1], lda, 
			    dwork, i__4);
	    dq_to_panel('U', ib, &a[i__ + i__ * a_dim1], lda, t+ib*ib);
	    //-----------------------------------------------------------------
	    ...
	    //-----------------------------------------------------------------
	    /*
	    lapackf77_dlarfb(side, trans, "Forward", "Columnwise", &mi, &ni, &ib, 
