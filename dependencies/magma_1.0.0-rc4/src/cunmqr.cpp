...
/*  -- MAGMA (version 1.0) --
    ...
    cublasSetMatrix( m, n, sizeof(cuFloatComplex), c, ldc, dc, ldc);
    dc -= (1 + m);
    //-------------------------------------------------------------------------
	...
	*info = -12;
    ...
    if (nb < nbmin || nb >= k) 
      {
	/* Use unblocked code */
	lapackf77_cunm2r(side_, trans_, &m, &n, &k, &a[a_offset], &lda, &tau[1], 
	...
	for (i__ = i1; i3 < 0 ? i__ >= i2 : i__ <= i2; i__ += i3) 
	  {
	    ...
	    i__4 = nq - i__ + 1;
	    lapackf77_clarft("F", "C", &i__4, &ib, &a[i__ + i__ * a_dim1], &lda, 
		    &tau[i__], t, &ib);

	    // TTT ------------------------------------------------------------
	    cpanel_to_q('U', ib, &a[i__ + i__ * a_dim1], lda, t+ib*ib);
	    cublasSetMatrix(i__4, ib, sizeof(cuFloatComplex),
			    &a[i__ + i__ * a_dim1], lda, 
			    dwork, i__4);
	    cq_to_panel('U', ib, &a[i__ + i__ * a_dim1], lda, t+ib*ib);
	    //-----------------------------------------------------------------
	    ...
	    //-----------------------------------------------------------------
	    /*
	    lapackf77_clarfb(side, trans, "Forward", "Columnwise", &mi, &ni, &ib, 
