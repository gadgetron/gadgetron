...
magma_clatrd(char uplo, magma_int_t n_, magma_int_t nb_, 
	     cuFloatComplex *a,  magma_int_t lda_, 
	     float *e, cuFloatComplex *tau, 
	     cuFloatComplex *w,  magma_int_t ldw_,
	     cuFloatComplex *da, magma_int_t ldda_, 
	     cuFloatComplex *dw, magma_int_t lddw_)
{
/*  -- MAGMA (version 1.0) --
       Univ. of Tennessee, Knoxville
       Univ. of California, Berkeley
...
d in   
    ...
    if (lapackf77_lsame(uplo_, "U")) {
      ...
      fprintf(stderr, "clatrd: uplo has to be 'L'; 'U' not implemented \n");
      exit(-1);
	...
	if (i__ < *n) {
	  /* Update A(1:i,i) */
	  i__2 = *n - i__;
	  blasf77_cgemv("No transpose", &i__, &i__2, &c_neg_one, 
	...
	if (i__ > 1) {
	  ...
	  i__2 = i__ - 1;
	  lapackf77_clarfg(&i__2, &a[i__ - 1 + i__ * a_dim1], &a[i__ * a_dim1 + 1], 
		  &c__1, &tau[i__ - 1]);

	  e[i__ - 1] = MAGMA_C_GET_X( a[i__ - 1 + i__ * a_dim1] );
	  a[i__ - 1 + i__ * a_dim1] = c_one;
  
	  /* Compute W(1:i-1,i) */
	  i__2 = i__ - 1;
	  blasf77_chemv("Upper", &i__2, &c_one, &a[a_offset], lda, 
	  ...
	  i__2 = i__ - 1;
	  blasf77_cscal(&i__2, &tau[i__ - 1], &w[iw * w_dim1 + 1], &c__1);
	  i__2 = i__ - 1;
#if defined(PRECISION_z) || defined(PRECISION_c)
	  blasf77_cdotc(&value, &i__2, &w[iw*w_dim1+1], &c__1, &a[i__ * a_dim1 + 1], &c__1);
	  alpha = tau[i__ - 1] * -.5f * value;
#else
	  alpha = tau[i__ - 1] * -.5f * 
	    blasf77_cdotc(&i__2, &w[iw*w_dim1+1], &c__1, &a[i__ * a_dim1 + 1], &c__1);
#endif
	  i__2 = i__ - 1;
	  blasf77_caxpy(&i__2, &alpha, &a[i__ * a_dim1 + 1], &c__1, 
		 &w[iw * w_dim1 + 1], &c__1);
	}
      }
      ...
      for (i__ = 1; i__ <= *nb; ++i__) 
	{
	  ...
	  i__3 = i__ - 1;
          #if defined(PRECISION_z) || defined(PRECISION_c)
	      lapackf77_clacgv(&i__3, &w[i__ + w_dim1], ldw);
          #endif
	  blasf77_cgemv("No transpose", &i__2, &i__3, &c_neg_one, 
			&a[i__ + a_dim1], lda, 
	  ...
	  if (i__ < *n) 
	    {
	      ...
	      cublasChemv('L', i__2, c_one, da+ (i__-1)+1 + ((i__-1)+1) * (*ldda),
			  *ldda, da+ (i__-1)+1 + (i__-1)* a_dim1, c__1, c_zero,
			  dw+ i__ + 1 + i__ * w_dim1, c__1);
	  
	      // 2. Start putting the result back (asynchronously)
	      cudaMemcpy2DAsync(w +i__+1+i__*w_dim1, w_dim1*sizeof(cuFloatComplex),
				dw+i__+1+i__*w_dim1, w_dim1*sizeof(cuFloatComplex),
	      ...
	      if (i__3!=0)
		blasf77_caxpy(&i__2, &c_one, f, &c__1, 
	      ...
	      blasf77_cgemv("No transpose", &i__2, &i__3, &c_neg_one,
			    ...
			    &w[i__ + 1 + i__ * w_dim1], &c__1);
	      blasf77_cscal(&i__2, &tau[i__], &w[i__ + 1 + i__ * w_dim1], &c__1);
              #if defined(PRECISION_z) || defined(PRECISION_c)
  	          blasf77_cdotc(&value, &i__2, &w[i__ +1+ i__ * w_dim1], 
				  &c__1, &a[i__ +1+ i__ * a_dim1], &c__1);
		  alpha = tau[i__]* -.5f * value;
              #else
	          alpha = tau[i__]* -.5f*blasf77_cdotc(&i__2, &w[i__ +1+ i__ * w_dim1], 
						     &c__1, &a[i__ +1+ i__ * a_dim1],
						     &c__1);
              #endif
	      blasf77_caxpy(&i__2, &alpha, &a[i__ + 1 + i__ * a_dim1], &c__1, 
			    &w[i__ + 1 + i__ * w_dim1], &c__1);
	    }
	}
    }
