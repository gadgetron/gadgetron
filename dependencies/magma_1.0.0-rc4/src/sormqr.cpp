...
/*  -- MAGMA (version 1.0) --
	...
	*info = -12;
    ...
    if (nb < nbmin || nb >= k) 
      {
	/* Use unblocked code */
	lapackf77_sorm2r(side_, trans_, &m, &n, &k, &a[a_offset], &lda, &tau[1], 
	...
	for (i__ = i1; i3 < 0 ? i__ >= i2 : i__ <= i2; i__ += i3) 
	  {
	    ...
	    i__4 = nq - i__ + 1;
	    lapackf77_slarft("F", "C", &i__4, &ib, &a[i__ + i__ * a_dim1], &lda, 
		    &tau[i__], t, &ib);
	    ...
	    //-----------------------------------------------------------------
	    /*
	    lapackf77_slarfb(side, trans, "Forward", "Columnwise", &mi, &ni, &ib, 
