...
*/
    ...
    if( m % dgemv_bs == 0 ) {
	    if( uplo == 'L' || uplo == 'l'){	
		    l_clanhe_special <<<grid, threads>>> (m, A, lda, Y);
	    }
            else{
		    u_clanhe_special <<<grid, threads>>> (m, A, lda,  Y);
	    } 
	    ...
	    int  m_mod_32 = m%32 ;  
	    if( uplo == 'L' || uplo == 'l'){
		    l_clanhe_generic <<<grid, threads>>> (m, A, lda, Y , m_full_block , m_mod_32);
	    }	
	    else{
		    u_clanhe_generic <<<grid, threads>>> (m, A, lda, Y , m_full_block , m_mod_32);
	    }	
    }
}
