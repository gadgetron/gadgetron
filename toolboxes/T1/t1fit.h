...
/**
 * Fits a T1 map using the 2 parameter model, and calculates the sign by trying all combinations
 * @param data Data of shape (X,Y,TI)
 * @param TI Inversion times
 * @return Magnitude (A) and T1 mapping
 */
T1_2param fit_T1_2param(const hoNDArray<std::complex<float>>& data, const std::vector<float>& TI);
 ...
 * on a two parameter T1 fit, and registering it to the original data using intensity based
 *registration.
 ...
 **/
hoNDArray<vector_td<float, 2>> t1_registration(const hoNDArray<std::complex<float>>& data,
...
hoNDArray<vector_td<float, 2>> t1_registration(const hoNDArray<std::complex<float>>& data,
                                               const std::vector<float>& TI,
                                               hoNDArray<vector_td<float,2>> initial_vfield,
                                               unsigned int iterations = 5,
                                               registration_params params = {});
hoNDArray<vector_td<float, 2>>
multi_scale_t1_registration(const hoNDArray<std::complex<float>>& data,
                            const std::vector<float>& TI, unsigned int levels = 3,
                            unsigned int iterations = 5, registration_params params = {});

hoNDArray<std::complex<float>> deform_groups(const hoNDArray<std::complex<float>>& data,
                                             const hoNDArray<vector_td<float, 2>>& vector_field);

hoNDArray<float> deform_groups(const hoNDArray<float>& data,
                                             const hoNDArray<vector_td<float, 2>>& vector_field);
...
hoNDArray<float> phase_correct(const hoNDArray<std::complex<float>>& data,
                               const std::vector<float>& TI);

hoNDArray<vector_td<float, 2>> register_groups_CMR(const hoNDArray<float>& phase_corrected_data,
                                                   const hoNDArray<float>& predicted, float hilbert_strength=12.0f);
/**
 * Preforms registration on a T1 dataset by iteratively creating synthetic data. Uses the CMR motion
 * correction code, and returns the deformed data
 * @param data Input data of shape (X,Y,TI)
 * @param TI inversion times
 * @param iterations Number of iterations between synthetic and registration
 * @return Motion corrected data
 */
hoNDArray<vector_td<float, 2>> t1_moco_cmr(const hoNDArray<std::complex<float>>& data,
