...
namespace Gadgetron{
  ...
  template <class COMPLEX_ARRAY_TYPE, unsigned long long D> class convolutionOperator : public linearOperator<COMPLEX_ARRAY_TYPE>
  {  
  protected:
  ...
  public:
	...
	throw std::runtime_error( "convolutionOperator::mult_MH_M failed : in/out image dimensions mismatch the kernel");
      ...
      if( use_oversampling ){
	boost::shared_ptr< std::vector<unsigned long long> > osdims = kernel_->get_dimensions();
	...
	*out = *in;
	...
	use_oversampling = true;
      ...
      if( use_oversampling ){
	boost::shared_ptr< std::vector<unsigned long long> > osdims = adjoint_kernel_->get_dimensions();
	tmp_out = new COMPLEX_ARRAY_TYPE(osdims);
	pad<ELEMENT_TYPE,D>( in, tmp_out );
      }
      else if( accumulate ){
  ...
  protected:
    ...
    virtual void operator_crop( COMPLEX_ARRAY_TYPE *in, COMPLEX_ARRAY_TYPE *out ){
      typename uintd<D>::Type offset = from_std_vector<unsigned long long,D>(*(in->get_dimensions().get()))>>2;
