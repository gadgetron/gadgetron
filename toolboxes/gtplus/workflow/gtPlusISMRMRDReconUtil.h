...
public:
    ...
    bool computeKLFilter(const hoNDArray<T>& data, unsigned long long numOfModesKept, hoNDArray<T>& dataKLF);
    ...
    // dstSize >= srcSize
    bool zpadRange(unsigned long long srcSize, unsigned long long dstSize, unsigned long long& start, unsigned long long& end);

    // pad the first two dimensions around its center, other dimensions are kept unchanged
    bool zeropad2D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, hoNDArray<T>& dataPadded);

    // pad first three dimensions array around its center, other dimensions are kept unchanged
    bool zeropad3D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataPadded);

    // the dataPadded is not pre cleared to fill with zeros
    bool zeropad3DNoPresetZeros(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataPadded);

    // cut the center part
    bool cutpad2D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, hoNDArray<T>& dataCut);
    bool cutpad3D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataCut);
    ...
    bool generateSymmetricFilter(unsigned long long len, hoNDArray<T>& filter, ISMRMRDKSPACEFILTER filterType, double sigma, unsigned long long width);
    ...
    bool generateAsymmetricFilter(unsigned long long len, unsigned long long start, unsigned long long end, hoNDArray<T>& filter, ISMRMRDKSPACEFILTER filterType, unsigned long long width, bool densityComp=false);

    // generate ref data filter
    bool generateSymmetricFilterForRef(unsigned long long len, unsigned long long start, unsigned long long end, hoNDArray<T>& filter, ISMRMRDKSPACEFILTER filterType, double sigma, unsigned long long width);

    // find the symmetric sampled region
    bool findSymmetricSampledRegion(unsigned long long start, unsigned long long end, unsigned long long center, unsigned long long& startSym, unsigned long long& endSym);
    ...
    bool detectSampledRegion2D(const hoNDArray<T>& data, unsigned long long& startRO, unsigned long long& endRO, unsigned long long& startE1, unsigned long long& endE1);
    bool detectSampledRegion3D(const hoNDArray<T>& data, unsigned long long& startRO, unsigned long long& endRO, unsigned long long& startE1, unsigned long long& endE1, unsigned long long& startE2, unsigned long long& endE2);
    ...
    bool averageKSpace4D(const hoNDArray<T>& data, hoNDArray<T>& ave, std::vector<unsigned long long>& sampledTimes);
    ...
    bool averageKSpace5D(const hoNDArray<T>& data, hoNDArray<T>& ave, hoNDArray<unsigned long long>& sampledTimes);

    // sampled region along E1
    // data: [RO E1 CHA N]
    bool detectSampledRegionE1(const hoNDArray<T>& data, unsigned long long& startE1, unsigned long long& endE1);

    // sampled times along E1, if not sampled, sampledTimes[e1] == 0
    bool detectSampledTimesE1(const hoNDArray<T>& data4D, std::vector<unsigned long long>& sampledTimes);

    // sampled region along E1 and E2
    // data: [RO E1 E2 CHA N]
    bool detectSampledRegionE1E2(const hoNDArray<T>& data, unsigned long long& startE1, unsigned long long& endE1, unsigned long long& startE2, unsigned long long& endE2);

    // sampled times along E1 and E2, if not sampled, sampledTimes(e1, e2) == 0
    // data5D: [RO E1 E2 CHA N]
    bool detectSampledTimesE1E2(const hoNDArray<T>& data5D, hoNDArray<unsigned long long>& sampledTimes);

    // copy along E1
    bool copyAlongE1(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startE1, unsigned long long endE1);

    // copy along RO and E1
    bool copyAlongROE1(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startRO, unsigned long long endRO, unsigned long long startE1, unsigned long long endE1);

    // copy along RO, E1 and E2
    bool copyAlongROE1E2(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startRO, unsigned long long endRO, unsigned long long startE1, unsigned long long endE1, unsigned long long startE2, unsigned long long endE2);
    ...
    bool copyAlongROE1TransitionBand(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startRO, unsigned long long endRO, 
    ...
    bool copyAlongROE1E2TransitionBand(const hoNDArray<T>& src, hoNDArray<T>& dst, 
                                    unsigned long long startRO, unsigned long long endRO, 
                                    unsigned long long startE1, unsigned long long endE1, 
                                    unsigned long long startE2, unsigned long long endE2, 
                                    unsigned long long transBandRO, unsigned long long transBandE1, 
                                    unsigned long long transBandE2);
    ...
    bool findDimIndex(const std::vector<DimensionRecordType>& dimStartingIndexes, ISMRMRDDIM dim, unsigned long long ind);
    ...
    bool extractSubArrayForDim(const hoNDArray<T>& x, hoNDArray<T>& r, ISMRMRDDIM& dim, unsigned long long value, bool lessEqual);
    // if lessEqual ==  true, [0:value1 0:value2] are extracted for dim
    bool extractSubArrayForDim(const hoNDArray<T>& x, hoNDArray<T>& r, ISMRMRDDIM& dim1, unsigned long long value1, ISMRMRDDIM& dim2, unsigned long long value2, bool lessEqual);

    // extract sub array for two dimensions
    // [0:value1] and [value2]
    bool extractSubArrayForDim1LessEqualDim2Equal(const hoNDArray<T>& x, hoNDArray<T>& r, ISMRMRDDIM& dim1, unsigned long long value1, ISMRMRDDIM& dim2, unsigned long long value2);
    ...
    void findStartEndRO(unsigned long long centre_column, unsigned long long samples, int& startRO, int& endRO);

    // find RO ranges from centre_column and number of samples after zero-filling
    void findStartEndROAfterZeroFilling(unsigned long long centre_column, unsigned long long samples_zerofilled, int& startRO, int& endRO);
};
...
public:
    ...
    bool zpadResize2D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, hoNDArray<T>& dataResized);
    // data: [RO E1 SLC E2 CON PHS REP SET], 1, 2 and 4th dimensions are resized
    bool zpadResize3D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataResized);

    // zero-padding resize with kspace as input
    bool zpadResize2DOnKSpace(const hoNDArray<T>& kspace, unsigned long long sizeX, unsigned long long sizeY, hoNDArray<T>& dataResized);
    bool zpadResize3DOnKSpace(const hoNDArray<T>& kspace, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataResized);
    ...
    // data: the 1st and 2rd dimensions are resized
    bool zpadResize2DFilter(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, const hoNDArray<T>& filter2D, hoNDArray<T>& dataResized);
    // filter3D: 3D array for kspace filter
    // data: [RO E1 SLC E2 CON PHS REP SET], 1, 2 and 4th dimensions are resized
    bool zpadResize3DFilter(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, const hoNDArray<T>& filter3D, hoNDArray<T>& dataResized);
    ...
    bool coilMap2DNIH(const hoNDArray<T>& data, hoNDArray<T>& coilMap, ISMRMRDCOILMAPALGO algo, unsigned long long ks=11, unsigned long long power=3, unsigned long long iterNum=5, typename realType<T>::Type thres=1e-3);
    // data: in image domain, at least 4D [RO E1 E2 CHA], the coil map will be estimated for every 2D kspace [RO E1 CHA] across E2
    bool coilMap3DNIH(const hoNDArray<T>& data, hoNDArray<T>& coilMap, ISMRMRDCOILMAPALGO algo, unsigned long long ks=7, unsigned long long power=3, unsigned long long iterNum=5, typename realType<T>::Type thres=1e-3);
    // data: [RO E1 CHA], only 3D array
    bool coilMap2DNIHInner(const hoNDArray<T>& data, hoNDArray<T>& coilMap, unsigned long long ks, unsigned long long power);
