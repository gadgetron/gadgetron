...
public:
    ...
    virtual bool performRecon(gtPlusReconWorkOrder<T>* workOrder)
    {
        ...
        if ( v <= 0 )
        {
            ...
            boost::shared_ptr< std::vector<unsigned long long> > dims = workOrder->data_.get_dimensions();
    ...
    virtual bool performCalibImpl(const hoNDArray<T>& ref_src, const hoNDArray<T>& ref_dst, gtPlusReconWorkOrder2DT<T>* workOrder2DT, unsigned long long n, unsigned long long usedS);
    ...
    virtual bool prepRef(gtPlusReconWorkOrder2DT<T>* workOrder2DT, const hoNDArray<T>& ref, 
                    ...
                    int startRO, int endRO, int startE1, int endE1, unsigned long long dataE1);
    ...
    virtual bool estimateJobSize(gtPlusReconWorkOrder<T>* workOrder, unsigned long long maxNumOfBytesPerJob, unsigned long long overlapBetweenJobs, unsigned long long numOfNodes, unsigned long long& jobSize);
...
protected:
    ...
    unsigned long long startE1_;
    unsigned long long endE1_;
};
                                        ...
                                        int startRO, int endRO, int startE1, int endE1)
{
    try
    {
        ...
        unsigned long long RO = ref.get_size(0);
        unsigned long long E1 = ref.get_size(1);
...
bool gtPlusReconWorker2DT<T>::prepRefByAveragingCrossN(gtPlusReconWorkOrder2DT<T>* workOrder2DT, const hoNDArray<T>& ref, bool averageAllRef, int numOfModes, hoNDArray<T>& refRecon)
{
    try
    {
        unsigned long long RO = ref.get_size(0);
        unsigned long long E1 = ref.get_size(1);
        unsigned long long CHA = ref.get_size(2);
        unsigned long long N = ref.get_size(3);
        unsigned long long S = ref.get_size(4);
        ...
        else if ( averageAllRef && (numOfModes>=1) && (numOfModes<=N-1) )
        {
            ...
            for ( s=0; s<S; s++ )
            {
                hoMatrix<T> A(N, RO*E1*CHA, const_cast<T*>(ref.begin()+s*RO*E1*CHA*N));
                hoMatrix<T> A_KLF(N, RO*E1*CHA, refKLF.begin()+s*RO*E1*CHA*N);
        ...
        else if ( !averageAllRef && (numOfModes>=1) && (numOfModes<=N-1) )
        {
            ...
            {
                hoMatrix<T> A(N, RO*E1*CHA, const_cast<T*>(ref.begin()+s*RO*E1*CHA*N));
                hoMatrix<T> A_KLF(N, RO*E1*CHA, refRecon.begin()+s*RO*E1*CHA*N);
                    ...
                    int startRO, int endRO, int startE1, int endE1, unsigned long long dataE1)
{
    try
    {
        unsigned long long dataRO = workOrder2DT->data_.get_size(0);
        unsigned long long dataS = workOrder2DT->data_.get_size(4);

        unsigned long long RO = ref.get_size(0);
        unsigned long long E1 = ref.get_size(1);
        unsigned long long srcCHA = ref.get_size(2);
        unsigned long long N = ref.get_size(3);
        unsigned long long S = ref.get_size(4);
        ...
        else if ( workOrder2DT->CalibMode_ == ISMRMRD_interleaved )
        {
            ...
            {
                std::vector<unsigned long long> crop_offset(5), crop_size(5);
                ...
                || workOrder2DT->CalibMode_ == ISMRMRD_external )
        {
            ...
            std::vector<unsigned long long> crop_offset(5);
            crop_offset[0] = 0;
            ...
            std::vector<unsigned long long> crop_size(5);
            ...
            if ( S < dataS )
            {
                ...
                unsigned long long refReconN4D = refRecon.get_size(0)*refRecon.get_size(1)*refRecon.get_size(2)*refRecon.get_size(3);
                unsigned long long refCoilMapN4D = refCoilMap.get_size(0)*refCoilMap.get_size(1)*refCoilMap.get_size(2)*refCoilMap.get_size(3);
...
bool gtPlusReconWorker2DT<T>::coilCompression(gtPlusReconWorkOrder2DT<T>* workOrder2DT)
{
    ...
    try
    {
        unsigned long long RO = workOrder2DT->ref_recon_.get_size(0);
        unsigned long long E1 = workOrder2DT->ref_recon_.get_size(1);
        unsigned long long srcCHA = workOrder2DT->ref_recon_.get_size(2);
        unsigned long long N = workOrder2DT->ref_recon_.get_size(3);
        unsigned long long S = workOrder2DT->ref_recon_.get_size(4);
        ...
        if ( workOrder2DT->coil_compression_ )
        {
            ...
            if ( workOrder2DT->coilCompressionCoef_->size()!=S )
            {
                if ( workOrder2DT->same_coil_compression_coeff_allS_ )
                {
                    ...
                    std::vector<unsigned long long> allSDim(4);
                    allSDim[0] = RO;
                    allSDim[1] = E1;
                    allSDim[2] = srcCHA;
                    allSDim[3] = N*S;
                ...
                else
                {
                    ...
                    unsigned long long num_modesKept = srcCHA;
...
estimateCoilMap(gtPlusReconWorkOrder2DT<T>* workOrder2DT, const hoNDArray<T>& ref_src, const hoNDArray<T>& ref_dst, const hoNDArray<T>& ref_coil_map_dst)
{
    try
    {
        unsigned long long RO = workOrder2DT->data_.get_size(0);
        unsigned long long E1 = workOrder2DT->data_.get_size(1);
        unsigned long long N = workOrder2DT->data_.get_size(3);
        unsigned long long S = workOrder2DT->data_.get_size(4);

        unsigned long long srcCHA = ref_src.get_size(2);

        unsigned long long refRO = ref_dst.get_size(0);
        unsigned long long refE1 = ref_dst.get_size(1);
        unsigned long long refN = ref_dst.get_size(3);
        unsigned long long dstCHA = ref_coil_map_dst.get_size(2);

        bool same_combinationcoeff_allS = false;
        unsigned long long whichS_combinationcoeff = 0;
            ...
            || (workOrder2DT->coilMap_->get_size(4)!=S) )
        {
            ...
            {
                unsigned long long usedS = whichS_combinationcoeff;
                ...
                GADGET_CHECK_RETURN_FALSE(gtPlusISMRMRDReconUtilComplex<T>().coilMap2DNIH(buffer2DT_, 
                        ...
                        workOrder2DT->csm_powermethod_num_, workOrder2DT->csm_iter_num_, workOrder2DT->csm_iter_thres_));
    ...
    try
    {
                ...
                || (workOrder2DT->kernelIm_->get_size(5)!=S) )
        {
            ...
            else
            {
                ...
                {
                    for ( unsigned long long n=0; n<refN; n++ )
                    {
...
template <typename T> 
bool gtPlusReconWorker2DT<T>::
performCalibImpl(const hoNDArray<T>& , const hoNDArray<T>& , gtPlusReconWorkOrder2DT<T>* , unsigned long long , unsigned long long )
{
    return true;
}
...
bool gtPlusReconWorker2DT<T>::unmixCoeff(const hoNDArray<T>& kerIm, const hoNDArray<T>& coilMap, hoNDArray<T>& unmixCoeff, hoNDArray<T>& gFactor)
{
    try
    {
        unsigned long long RO = kerIm.get_size(0);
        unsigned long long E1 = kerIm.get_size(1);
        unsigned long long srcCHA = kerIm.get_size(2);
        unsigned long long dstCHA = kerIm.get_size(3);
        ...
        #pragma omp parallel default(none) private(src) shared(RO, E1, srcCHA, dstCHA, pKerIm, pCoilMap, pCoeff, dim)
        {
            ...
            for ( src=0; src<(int)srcCHA; src++ )
            {
                ...
                for ( unsigned long long dst=0; dst<dstCHA; dst++ )
                {
...
bool gtPlusReconWorker2DT<T>::applyImageDomainKernelImage(const hoNDArray<T>& aliasedIm, const hoNDArray<T>& kerIm, hoNDArray<T>& kerImBuffer, hoNDArray<T>& complexIm)
{
    try
    {
        ...
        boost::shared_ptr< std::vector<unsigned long long> > dim = aliasedIm.get_dimensions();

        std::vector<unsigned long long> dimIm(*dim);
        dimIm[2] = dstCHA;
        ...
        std::vector<unsigned long long> dimIm3D(3);
        dimIm3D[0] = RO;
        dimIm3D[1] = E1;
        dimIm3D[2] = dstCHA;

        unsigned long long num = aliasedIm.get_number_of_elements()/ (RO*E1*srcCHA);
...
bool gtPlusReconWorker2DT<T>::afterUnwrapping(gtPlusReconWorkOrder2DT<T>* workOrder2DT)
{
    try
    {
        ...
        unsigned long long whichS_coilmap = 0;

        unsigned long long RO = workOrder2DT->kernelIm_->get_size(0);
        unsigned long long E1 = workOrder2DT->kernelIm_->get_size(1);
        unsigned long long srcCHA = workOrder2DT->kernelIm_->get_size(2);
        unsigned long long dstCHA = workOrder2DT->kernelIm_->get_size(3);
        unsigned long long N = workOrder2DT->data_.get_size(3);
        unsigned long long S = workOrder2DT->data_.get_size(4);
            ...
            if ( averageallN_coilmap )
            {
                ...
                else
                {
                    ...
                    if ( same_coilmap_allS )
                    {
                        ...
                        hoMatrix<T> A(N, RO*E1*dstCHA, buffer2DT_.begin()+whichS_coilmap*RO*E1*dstCHA*N);
                        hoMatrix<T> A_KLF(N, RO*E1*dstCHA, buffer2DT_unwrapping_.begin());
                    ...
                    else
                    {
                        ...
                        if ( numOfModesKept>0 && numOfModesKept<dstCHA )
                        {
                            for ( s=0; s<S; s++ )
                            {
                                hoMatrix<T> A(N, RO*E1*dstCHA, buffer2DT_.begin()+s*RO*E1*dstCHA*N);
                                hoMatrix<T> A_KLF(N, RO*E1*dstCHA, buffer2DT_unwrapping_.begin()+s*RO*E1*dstCHA*N);
                        ...
                        for ( s=0; s<S; s++ )
                        {
                            hoNDArray<T> aveComplexImS(RO, E1, dstCHA, aveComplexIm.begin()+s*RO*E1*dstCHA);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ( same_coilmap_allS )
                    {
                        ...
                        GADGET_CHECK_RETURN_FALSE(gtPlusISMRMRDReconUtilComplex<T>().coilMap2DNIH(complexImS, coilMapS, workOrder2DT->coil_map_algorithm_, workOrder2DT->csm_kSize_, workOrder2DT->csm_powermethod_num_, workOrder2DT->csm_iter_num_, workOrder2DT->csm_iter_thres_));
                        GADGET_CHECK_RETURN_FALSE(repmatLastDimension(*workOrder2DT->coilMap_, whichS_coilmap));
                        GADGET_EXPORT_ARRAY_COMPLEX(debugFolder_, gt_exporter_, *workOrder2DT->coilMap_, "coilMap_fullres");
                    }
                    else
                    {
                        GADGET_CHECK_RETURN_FALSE(gtPlusISMRMRDReconUtilComplex<T>().coilMap2DNIH(buffer2DT_, *workOrder2DT->coilMap_, workOrder2DT->coil_map_algorithm_, workOrder2DT->csm_kSize_, workOrder2DT->csm_powermethod_num_, workOrder2DT->csm_iter_num_, workOrder2DT->csm_iter_thres_));
                        GADGET_EXPORT_ARRAY_COMPLEX(debugFolder_, gt_exporter_, *workOrder2DT->coilMap_, "coilMap_fullres");
                    }
...
bool gtPlusReconWorker2DT<T>::pickHighestSignalForN(const hoNDArray<T>& data, hoNDArray<T>& res)
{
    try
    {
        ...
        unsigned long long S = data.get_size(4);
...
bool gtPlusReconWorker2DT<T>::performPartialFourierHomodyneRecon(gtPlusReconWorkOrder2DT<T>& workOrder2DT, hoNDArray<T>& kspace)
{
    try
    {
        unsigned long long RO = kspace.get_size(0);
        unsigned long long E1 = kspace.get_size(1);
        unsigned long long CHA = kspace.get_size(2);
        unsigned long long N = kspace.get_size(3);
        unsigned long long S = kspace.get_size(4);
        ...
        {
            unsigned long long width_RO = std::floor(0.1*RO);
            unsigned long long width_E1 = std::floor(0.1*E1);
...
bool gtPlusReconWorker2DT<T>::performPartialFourierFengHuangRecon(gtPlusReconWorkOrder2DT<T>& workOrder2DT, hoNDArray<T>& kspace)
{
    try
    {
        ...
        hoNDArray<T> src, dst;
        std::vector<unsigned long long> start(5), size(5);
...
bool gtPlusReconWorker2DT<T>::calibFengHuang(gtPlusReconWorkOrder2DT<T>& workOrder2DT, const hoNDArray<T>& src, const hoNDArray<T>& dst, ho6DArray<T>& kernel)
{
    try
    {
        ...
        unsigned long long RO = src.get_size(0);
        unsigned long long E1 = src.get_size(1);
        unsigned long long srcCHA = src.get_size(2);
        unsigned long long N = src.get_size(3);
        unsigned long long S = src.get_size(4);

        unsigned long long kx = workOrder2DT.partialFourier_FengHuang_kSize_RO_;
        unsigned long long ky = workOrder2DT.partialFourier_FengHuang_kSize_E1_;
        ...
        unsigned long long startRO = halfKx;
        unsigned long long endRO = RO - halfKx - 1;

        unsigned long long startE1 = halfKy;
        unsigned long long endE1 = E1 - halfKy - 1;
        ...
        #endif
        {
            ...
            for ( ii=0; ii<num; ii ++ )
            {
                ...
                unsigned long long ro, e1, row(0);
                int x, y;
                                                ...
                                                int startRO, int endRO, int startE1, int endE1, ho6DArray<T>& kernel)
{
    try
    {
        ...
        unsigned long long kx = kernel.get_size(0);
        unsigned long long ky = kernel.get_size(1);

        int halfKx = kx/2;
        int halfKy = ky/2;
        unsigned long long kerN = kernel.get_size(4);
        ...
        ho2DArray<unsigned long long> coeffX(rowD, colD);
        ho2DArray<unsigned long long> coeffY(rowD, colD);
        ...
        #endif
        {
            ...
            for ( ii=0; ii<num; ii ++ )
            {
                ...
                unsigned long long ro, e1, row, col;
...
template <typename T> 
bool gtPlusReconWorker2DT<T>::
estimateJobSize(gtPlusReconWorkOrder<T>* workOrder2DT, unsigned long long maxNumOfBytesPerJob, unsigned long long overlapBetweenJobs, unsigned long long numOfNodes, unsigned long long& jobSize)
{
    try
    {
        unsigned long long nodeN = numOfNodes;
        ...
        unsigned long long totalJobNum = N;
        jobSize = std::ceil( (double)(totalJobNum+overlapBetweenJobs*(nodeN-1))/(double)nodeN );

        unsigned long long numOfBytesPerJob = sizeof(T)*( RO*E1*srcCHA*dstCHA*jobSize + 2*RO*E1*srcCHA*jobSize );
