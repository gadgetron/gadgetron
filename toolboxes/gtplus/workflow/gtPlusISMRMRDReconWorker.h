...
struct gtPlusReconJob2DT : public SerializableObject
{
    ...
    unsigned long long job_index_startN_;
    unsigned long long job_index_endN_;
    unsigned long long job_index_S_;
...
template <typename T> 
bool gtPlusReconJob2DT<T>::serialize(char*& buf, unsigned long long& len) const
{
    ...
    try
    {
        ...
        unsigned long long lenKSpace, lenKernel, lenCoilMap, lenComplexIm, lenRes;
        ...
        // total length
        len = sizeof(gtPlusReconWorkOrderPara) + sizeof(unsigned long long)*3 + lenKSpace + lenKernel + lenCoilMap + lenComplexIm + lenRes;

        buf = new char[len];
        GADGET_CHECK_RETURN_FALSE( buf != NULL );

        unsigned long long offset = 0, currLen=0;

        currLen = sizeof(gtPlusReconWorkOrderPara);
        memcpy(buf+offset, &para, currLen);
        offset += currLen;

        currLen = sizeof(unsigned long long);
...
template <typename T> 
bool gtPlusReconJob2DT<T>::deserialize(char* buf, unsigned long long& len)
{
    try
    {
        gtPlusReconWorkOrderPara para;
        ...
        unsigned long long offset(sizeof(gtPlusReconWorkOrderPara)), currLen=0;
...
public:
    ...
    virtual bool splitReconJob(gtPlusReconWorkOrder<T>* workOrder2DT, hoNDArray<T>& kspace, hoNDArray<T>& ker, 
                        bool splitByS, unsigned long long jobN, unsigned long long jobMegaBytes, unsigned long long overlapN, 
                        std::vector<gtPlusReconJob2DT<T> >& jobList);

    virtual bool combineReconJob(gtPlusReconWorkOrder<T>* workOrder2DT, std::vector<gtPlusReconJob2DT<T> >& jobList, unsigned long long N, unsigned long long S);

    virtual bool createAReconJob(gtPlusReconWorkOrder<T>* workOrder2DT, hoNDArray<T>& kspace, hoNDArray<T>& ker, 
                            unsigned long long startN, unsigned long long endN, unsigned long long indS, gtPlusReconJob2DT<T>& job);

    // from the node computing power indexes, get the effective node number for job splitting
    virtual bool computeEffectiveNodeNumberBasedOnComputingPowerIndex(gtPlusReconWorkOrder<T>* workOrder, unsigned long long& numOfEffectiveNodes);

    // estimate the job size, given the maximal memory usage for every job
    virtual bool estimateJobSize(gtPlusReconWorkOrder<T>* workOrder, unsigned long long maxNumOfBytesPerJob, unsigned long long overlapBetweenJobs, unsigned long long numOfNodes, unsigned long long& jobSize) = 0;

    // given the number of nodes in a cloud and corresponding computing power indexes, spread the jobs on the nodes
    virtual bool scheduleJobForNodes(gtPlusReconWorkOrder<T>* workOrder2DT, unsigned long long numOfJobs, std::vector<int>& nodeIdForJob);
};
        ...
        unsigned long long startN, unsigned long long endN, unsigned long long indS, gtPlusReconJob2DT<T>& job)
{
    try
    {
        ...
        unsigned long long srcCHA = ker.get_size(2);
        unsigned long long dstCHA = ker.get_size(3);
        unsigned long long refN = ker.get_size(4);

        unsigned long long jobN = endN-startN+1;
        ...
        std::vector<gtPlusReconJob2DT<T> >& jobList)
{
    try
    {
        ...
        unsigned long long startN, endN;
...
combineReconJob(gtPlusReconWorkOrder<T>* workOrder2DT, std::vector<gtPlusReconJob2DT<T> >& jobList, unsigned long long N, unsigned long long S)
{
    try
    {
        unsigned long long RO = jobList[0].kspace.get_size(0);
        unsigned long long E1 = jobList[0].kspace.get_size(1);

        unsigned long long srcCHA = jobList[0].ker.get_size(2);
        unsigned long long dstCHA = jobList[0].ker.get_size(3);
        unsigned long long refN = jobList[0].ker.get_size(4);

        workOrder2DT->complexIm_.create(RO, E1, N, S);
        workOrder2DT->fullkspace_.create(RO, E1, dstCHA, N, S);

        unsigned long long ii, n, s;

        unsigned long long numOfJobs = jobList.size();
        ...
        {
            unsigned long long startN = jobList[ii].job_index_startN_;
            unsigned long long endN = jobList[ii].job_index_endN_;
            unsigned long long indS = jobList[ii].job_index_S_;
    ...
    {
        GADGET_ERROR_MSG("Errors in gtPlusReconWorker<T>::combineReconJob(gtPlusReconWorkOrder<T>* workOrder2DT, std::vector<gtPlusReconJob2DT<T> >& jobList, unsigned long long N, unsigned long long S) ... ");
        return false;
    }
...
template <typename T> 
bool gtPlusReconWorker<T>::
computeEffectiveNodeNumberBasedOnComputingPowerIndex(gtPlusReconWorkOrder<T>* workOrder, unsigned long long& numOfEffectiveNodes)
{
    try
    {
        unsigned long long numOfNodes = workOrder->gt_cloud_.size();
        ...
        numOfEffectiveNodes = (unsigned long long)(std::floor(totalPowerIndex/minPowerIndex));
    }
    catch(...)
    {
...
template <typename T> 
bool gtPlusReconWorker<T>::
scheduleJobForNodes(gtPlusReconWorkOrder<T>* workOrder, unsigned long long numOfJobs, std::vector<int>& nodeIdForJob)
{
    try
    {
        unsigned long long numOfNodes = workOrder->gt_cloud_.size();
        ...
        for ( unsigned long long ii=0; ii<numOfNodes; ii++ )
        {
    ...
    {
        GADGET_ERROR_MSG("Errors in gtPlusReconWorker<T>::scheduleJobForNodes(gtPlusReconWorkOrder<T>* workOrder2DT, unsigned long long numOfJobs, std::vector<int>& nodeIdForJob) ... ");
        return false;
    }
