...
public:
    ...
    virtual bool performUnwarppingImpl(gtPlusReconWorkOrder<T>* workOrder3DT, hoNDArray<T>& kspace, hoNDArray<T>& adj_forward_G_I, hoNDArray<T>& res, unsigned long long n);
    ...
    virtual bool splitJob(gtPlusReconWorkOrder3DT<T>* workOrder3DT, unsigned long long& jobN);
...
performCalibPrep(const hoNDArray<T>& ref_src, const hoNDArray<T>& ref_dst, WorkOrderType* workOrder3DT)
{
    ...
    unsigned long long srcCHA = ref_src.get_size(3);
    ...
    unsigned long long kRO = workOrder3DT->spirit_kSize_RO_;
    unsigned long long kE1 = workOrder3DT->spirit_kSize_E1_;
    unsigned long long kE2 = workOrder3DT->spirit_kSize_E2_;

    workOrder3DT->kernel_->create(kRO, kE1, kE2, srcCHA, dstCHA, 1, 1, 1, refN);

    unsigned long long jobN;
    ...
    if ( !splitJobs )
    {
        ...
        if ( gtPlus_mem_manager_ )
        {
            ...
            {
                workOrder3DT->kernelIm_->create(E1, E2, RO, srcCHA, dstCHA, refN, (T*)(gtPlus_mem_manager_->allocate(sizeof(T)*(unsigned long long)RO*E1*E2*srcCHA*dstCHA*refN)));
            }
        }
        else
        {
    ...
    {
        unsigned long long convKE1 = 2*kE1-1;
        unsigned long long convKE2 = 2*kE2-1;

        GADGET_CHECK_PERFORM(performTiming_, gt_timer3_.start("allocate image domain kernel only along RO ... "));
        if ( gtPlus_mem_manager_ )
        {
            if ( workOrder3DT->kernelIm_->get_number_of_elements() != (unsigned long long)RO*convKE1*convKE2*srcCHA*dstCHA*refN )
            {
                workOrder3DT->kernelIm_->create(convKE1, convKE2, RO, srcCHA, dstCHA, refN, (T*)(gtPlus_mem_manager_->allocate(sizeof(T)*(unsigned long long)RO*convKE1*convKE2*srcCHA*dstCHA*refN)));
            }
        }
        else
        {
...
template <typename T> 
bool gtPlusReconWorker3DTSPIRIT<T>::
splitJob(gtPlusReconWorkOrder3DT<T>* workOrder3DT, unsigned long long& jobN)
{
    unsigned long long RO = workOrder3DT->data_.get_size(0);
    unsigned long long E1 = workOrder3DT->data_.get_size(1);
    unsigned long long E2 = workOrder3DT->data_.get_size(2);
    ...
    unsigned long long jobMegaBytes = workOrder3DT->job_max_Megabytes_;
    ...
    if ( !splitJobs )
    {
        if ( jobMegaBytes>0 )
        {
            unsigned long long jobN = jobMegaBytes/(E1*E2*srcCHA*dstCHA*sizeof(T)/1024/1024);
...
performUnwrapping(gtPlusReconWorkOrder3DT<T>* workOrder3DT, const hoNDArray<T>& data_dst)
{
    try
    {
        ...
        unsigned long long kImE1 = workOrder3DT->kernelIm_->get_size(0);
        unsigned long long kImE2 = workOrder3DT->kernelIm_->get_size(1);
        unsigned long long kImRO = workOrder3DT->kernelIm_->get_size(2);
        unsigned long long srcCHA = workOrder3DT->kernelIm_->get_size(3);
        unsigned long long dstCHA = workOrder3DT->kernelIm_->get_size(4);
        ...
        // split the jobs
        unsigned long long jobMegaBytes = workOrder3DT->job_max_Megabytes_;
        unsigned long long jobN = workOrder3DT->job_num_of_N_;
        bool splitJobs = this->splitJob(workOrder3DT, jobN);
        unsigned long long maxNumOfBytesPerJob = jobMegaBytes*1024*1024;

        unsigned long long overlapN = workOrder3DT->job_overlap_;
        ...
        if ( splitJobs )
        {
            ...
            if ( !spirit_kernelIm_permuted_ )
            {
                ...
                unsigned long long kerN = kImE1*kImE2*srcCHA*dstCHA*kImRO*N;
                unsigned long long kerImSize = sizeof(T)*kerN;
                GADGET_MSG("SPIRIT - 3DT - image domain kernel size : " << kerImSize/1024.0/1024 << " MBytes ... ");
                unsigned long long maxFreeChunk = gtPlus_mem_manager_->maxFreeMemoryChunkSize();
            ...
            if ( runJobsOnCloud )
            {
                ...
                //unsigned long long numOfBytesPerJob = sizeof(T)*( E1*E2*srcCHA*dstCHA*jobN + 2*E1*E2*srcCHA*jobN );
...
performUnwarppingImplROPermuted(gtPlusReconWorkOrder<T>* workOrder3DT, hoNDArray<T>& kspace, hoNDArray<T>& ker, hoNDArray<T>& /*coilMap*/, hoNDArray<T>& res)
{
    try
    {
        unsigned long long E1 = kspace.get_size(0);
        unsigned long long E2 = kspace.get_size(1);
        unsigned long long RO = kspace.get_size(3);

        unsigned long long kerE1 = ker.get_size(0);
        unsigned long long kerE2 = ker.get_size(1);
        unsigned long long srcCHA = ker.get_size(2);
        unsigned long long dstCHA = ker.get_size(3);
        unsigned long long kerN = ker.get_size(5);
        ...
        if ( kerE1!=E1 || kerE2!=E2 )
        {
            ...
            if ( gtPlus_mem_manager_ )
            {
                ...
                kerImE1E2RO.create(E1, E2, srcCHA, dstCHA, RO, kerN, (T*)(gtPlus_mem_manager_->allocate(sizeof(T)*(unsigned long long)RO*E1*E2*srcCHA*dstCHA)));
            }
            else
            {
...
template <typename T> 
bool gtPlusReconWorker3DTSPIRIT<T>::
performUnwarppingImpl(gtPlusReconWorkOrder<T>* workOrder3DT, hoNDArray<T>& kspace, hoNDArray<T>& adj_forward_G_I, hoNDArray<T>& res, unsigned long long n)
{
    try
    {
        ...
        unsigned long long srcCHA = adj_forward_G_I.get_size(3);
        unsigned long long dstCHA = adj_forward_G_I.get_size(4);
        ...
        #pragma omp parallel default(none) private(t) shared(RO, E1, E2, srcCHA, dstCHA, workOrder3DT, NUM, resDecoupled, pKspaceIfftROPermuted, pG_I) if ( NUM > 6 ) num_threads( (NUM<16) ? NUM : 16 )
        {
            ...
            for ( t=0; t<NUM; t++ )
            {
                ...
                unsigned long long e1, e2, scha, dcha;
