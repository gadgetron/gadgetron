...
performUnwarppingImplROPermuted(gtPlusReconWorkOrder<T>* workOrder3DT, hoNDArray<T>& kspace, hoNDArray<T>& kernel, hoNDArray<T>& coilMap, hoNDArray<T>& res)
{
    try
    {
        ...
        unsigned long long kerE1 = kernel.get_size(0);
        unsigned long long kerE2 = kernel.get_size(1);
        unsigned long long srcCHA = kernel.get_size(2);
        unsigned long long dstCHA = kernel.get_size(3);
