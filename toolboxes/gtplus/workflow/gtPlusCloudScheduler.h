...
public:
    ...
    void setNumOfJobs(unsigned long long numOfJobs);

    void setUpNodes(unsigned long long numOfNodes);
    void setUpNodes(const std::vector<double>& nodeComputingPowerIndexes);

protected:

    // number of nodes
    unsigned long long num_of_nodes_;

    // number of jobs, for this simple scheduler, all jobs are considered to have equal sizes
    unsigned long long num_of_jobs_;
