...
KLT_eigenAnalysis(const hoMatrix<T>& data, hoMatrix<T>& eigenVectors, hoMatrix<T>& eigenValues)
{
    try
    {
        ...
        unsigned long long M = data.rows();
        unsigned long long N = data.cols();
...
KLT_applyEigen(const hoMatrix<T>& data, hoMatrix<T>& dataEigen, const hoMatrix<T>& eigenVectors)
{
    try
    {
        ...
        unsigned long long K = eigenVectors.cols();
...
KLT_applyEigen(const hoNDArray<T>& data, hoNDArray<T>& dataEigen, const hoMatrix<T>& eigenVectors)
{
    try
    {
        unsigned long long M = data.get_size(1);
        unsigned long long N = data.get_size(0);
...
KLT_numberOfKeptModes(const hoMatrix<T>& eigenValues, double thres, int& numOfModesKept)
{
    try
    {
        unsigned long long M = eigenValues.rows();
...
pruneEigenVectorMatrix(const hoMatrix<T>& eigenVectors, int numOfModesKept, hoMatrix<T>& eigenVectorsPruned)
{
    try
    {
        unsigned long long M = eigenVectors.rows();
        unsigned long long N = eigenVectors.cols();
...
computeKLTCoeff(const hoNDArray<T>& data, hoMatrix<T>& coeff, hoMatrix<T>& eigenValues, bool isChaLastDim)
{
    try
    {
        unsigned long long NDim = data.get_number_of_dimensions();
        ...
        if ( isChaLastDim )
        {
            unsigned long long CHA = data.get_size(NDim-1);
            unsigned long long N = data.get_number_of_elements()/CHA;
        ...
        else
        {
            ...
            {
                unsigned long long N = data.get_size(3);
            ...
            {
                std::vector<unsigned long long> dimOrder(NDim);
                ...
                unsigned long long num = data.get_number_of_elements()/CHA;
...
appyKLCoilCompressionCoeff(const hoNDArray<T>& data, const hoMatrix<T>& coeff, hoNDArray<T>& dataEigen, bool isChaLastDim)
{
    try
    {
        ...
        boost::shared_ptr< std::vector<unsigned long long> > dim = data.get_dimensions();

        unsigned long long dstCHA = coeff.cols();
        ...
        if ( isChaLastDim )
        {
            ...
            std::vector<unsigned long long> dimEigen(*dim);
        ...
        else
        {
            ...
            else if ( NDim >= 5 )
            {
                ...
                boost::shared_ptr< std::vector<unsigned long long> > dimP = dataP->get_dimensions();
                (*dimP)[NDim-1] = dstCHA;
...
applyKLCoilCompressionCoeff(const hoNDArray<T>& data, const std::vector<hoMatrix<T> >& coeff, hoNDArray<T>& dataEigen, bool isChaLastDim)
{
    try
    {
        ...
        unsigned long long LastDim = coeff.size();
        unsigned long long dstCHA = coeff[0].cols();
        ...
        unsigned long long LastDimData = data.get_size(NDim-1);
        boost::shared_ptr< std::vector<unsigned long long> > dim = data.get_dimensions();
        ...
        std::vector<unsigned long long> dimLastDim(NDim-1);
        for ( n=0; n<NDim-1; n++ )
        {
            dimLastDim[n] = (*dim)[n];
        }
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::computeKLFilter(const hoNDArray<T>& data, unsigned long long numOfModesKept, hoNDArray<T>& dataKLF)
{
    try
    {
        ...
        unsigned long long NDim = data.get_number_of_dimensions();
        unsigned long long M = data.get_size(NDim-1);
        unsigned long long N = data.get_number_of_elements()/M;
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
zpadRange(unsigned long long srcSize, unsigned long long dstSize, unsigned long long& start, unsigned long long& end)
{
    try
    {
        if ( srcSize >= dstSize )
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
zeropad2D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, hoNDArray<T>& dataPadded)
{
    try
    {
        unsigned long long RO = data.get_size(0);
        ...
        unsigned long long sRO, eRO, sE1, eE1;
        GADGET_CHECK_RETURN_FALSE(zpadRange(RO, sizeX, sRO, eRO));
        GADGET_CHECK_RETURN_FALSE(zpadRange(E1, sizeY, sE1, eE1));

        boost::shared_ptr< std::vector<unsigned long long> > dimPadded = data.get_dimensions();
        (*dimPadded)[0] = sizeX;
        (*dimPadded)[1] = sizeY;
        dataPadded.create(dimPadded);
        Gadgetron::clear(&dataPadded);

        unsigned long long num = data.get_number_of_elements()/(RO*E1);
        ...
        for ( n=0; n<(int)num; n++ )
        {
            for ( unsigned long long y=sE1; y<=eE1; y++ )
            {
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
zeropad3D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataPadded)
{
    try
    {
        unsigned long long RO = data.get_size(0);
        unsigned long long E1 = data.get_size(1);
        unsigned long long E2 = data.get_size(2);
        ...
        unsigned long long sRO, eRO, sE1, eE1, sE2, eE2;
        GADGET_CHECK_RETURN_FALSE(zpadRange(RO, sizeX, sRO, eRO));
        GADGET_CHECK_RETURN_FALSE(zpadRange(E1, sizeY, sE1, eE1));
        ...
        unsigned long long num = data.get_number_of_elements()/(RO*E1*E2);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
zeropad3DNoPresetZeros(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataPadded)
{
    try
    {
        unsigned long long RO = data.get_size(0);
        unsigned long long E1 = data.get_size(1);
        unsigned long long E2 = data.get_size(2);
        unsigned long long srcCHA = data.get_size(3);
        unsigned long long dstCHA = data.get_size(4);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
cutpad2D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, hoNDArray<T>& dataCut)
{
    try
    {
        unsigned long long RO = data.get_size(0);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
cutpad3D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataCut)
{
    try
    {
        unsigned long long RO = data.get_size(0);
        ...
        for ( n=0; n<(long long)num; n++ )
        {
            for ( unsigned long long z=sE2; z<=eE2; z++ )
            {
                for ( unsigned long long y=sE1; y<=eE1; y++ )
                {
...
compute2DFilterFromTwo1D(const hoNDArray<T>& fx, const hoNDArray<T>& fy, hoNDArray<T>& fxy)
{
    try
    {
        unsigned long long RO = fx.get_size(0);
        unsigned long long E1 = fy.get_size(0);
...
compute3DFilterFromThree1D(const hoNDArray<T>& fx, const hoNDArray<T>& fy, const hoNDArray<T>& fz, hoNDArray<T>& fxyz)
{
    try
    {
        unsigned long long RO = fx.get_size(0);
        unsigned long long E1 = fy.get_size(0);
        unsigned long long E2 = fz.get_size(0);
...
kspacefilterROE1E2(const hoNDArray<T>& data, const hoNDArray<T>& fROE1E2, hoNDArray<T>& dataFiltered)
{
    try
    {
        ...
        {
            unsigned long long NDim = data.get_number_of_dimensions();
            std::vector<unsigned long long> order(data.get_number_of_dimensions(), 1);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
generateSymmetricFilter(unsigned long long len, hoNDArray<T>& filter, ISMRMRDKSPACEFILTER filterType, double sigma, unsigned long long width)
{
    try
    {
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
generateAsymmetricFilter(unsigned long long len, unsigned long long start, unsigned long long end, hoNDArray<T>& filter, ISMRMRDKSPACEFILTER filterType, unsigned long long width, bool densityComp)
{
    try
    {
        if ( len == 0 ) return true;
        ...
        if ( densityComp )
        {
            unsigned long long startSym(0), endSym(len-1);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
generateSymmetricFilterForRef(unsigned long long len, unsigned long long start, unsigned long long end, 
        hoNDArray<T>& filter, ISMRMRDKSPACEFILTER filterType, double sigma, unsigned long long width)
{
    try
    {
        ...
        unsigned long long centerInd = len/2;

        unsigned long long lenFilter(0); // make a symmetric filter with zero at the center
        unsigned long long lenFilterEnd = 2*(end-centerInd)+1;
        unsigned long long lenFilterStart = 2*(centerInd-start)+1;
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
findSymmetricSampledRegion(unsigned long long start, unsigned long long end, unsigned long long center, unsigned long long& startSym, unsigned long long& endSym)
{
    GADGET_CHECK_RETURN_FALSE(end>=start);
    GADGET_CHECK_RETURN_FALSE(center>=start);
    GADGET_CHECK_RETURN_FALSE(end>=center);

    unsigned long long halfSizeStart = center - start;
    unsigned long long halfSizeEnd =  end - center;
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
detectSampledRegion2D(const hoNDArray<T>& data, unsigned long long& startRO, unsigned long long& endRO, unsigned long long& startE1, unsigned long long& endE1)
{
    try
    {
        unsigned long long NDim = data.get_number_of_dimensions();
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
detectSampledRegion3D(const hoNDArray<T>& data, unsigned long long& startRO, unsigned long long& endRO, unsigned long long& startE1, unsigned long long& endE1, unsigned long long& startE2, unsigned long long& endE2)
{
    try
    {
        unsigned long long NDim = data.get_number_of_dimensions();
        ...
        unsigned long long RO = mag.get_size(0);
        unsigned long long E1 = mag.get_size(1);
        unsigned long long E2 = mag.get_size(4);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
averageKSpace4D(const hoNDArray<T>& data, hoNDArray<T>& ave, std::vector<unsigned long long>& sampledTimes)
{
    try
    {
        unsigned long long NDim = data.get_number_of_dimensions();
        ...
        boost::shared_ptr< std::vector<unsigned long long> > dim = ave.get_dimensions();
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
averageKSpace5D(const hoNDArray<T>& data, hoNDArray<T>& ave, hoNDArray<unsigned long long>& sampledTimes)
{
    try
    {
        unsigned long long NDim = data.get_number_of_dimensions();
        ...
        unsigned long long E1 = data.get_size(1);
        unsigned long long E2 = data.get_size(2);
        unsigned long long CHA = data.get_size(3);
        unsigned long long N = data.get_size(4);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
detectSampledTimesE1(const hoNDArray<T>& data4D, std::vector<unsigned long long>& sampledTimes)
{
    try
    {
        unsigned long long NDim = data4D.get_number_of_dimensions();
        GADGET_CHECK_RETURN_FALSE(NDim>=2);

        unsigned long long RO = data4D.get_size(0);
        unsigned long long E1 = data4D.get_size(1);
        unsigned long long CHA = data4D.get_size(2);
        unsigned long long N = data4D.get_size(3);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
detectSampledRegionE1(const hoNDArray<T>& data, unsigned long long& startE1, unsigned long long& endE1)
{
    try
    {
        std::vector<unsigned long long> sampledTimes;
        GADGET_CHECK_RETURN_FALSE(detectSampledTimesE1(data, sampledTimes));

        unsigned long long E1 = sampledTimes.size();

        startE1 = E1-1;
        endE1 = 0;

        for ( unsigned long long e1=0; e1<E1; e1++ )
        {
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
detectSampledTimesE1E2(const hoNDArray<T>& data5D, hoNDArray<unsigned long long>& sampledTimes)
{
    try
    {
        unsigned long long NDim = data5D.get_number_of_dimensions();
        GADGET_CHECK_RETURN_FALSE(NDim>=3);

        unsigned long long RO = data5D.get_size(0);
        unsigned long long E1 = data5D.get_size(1);
        unsigned long long E2 = data5D.get_size(2);
        unsigned long long CHA = data5D.get_size(3);
        unsigned long long N = data5D.get_size(4);
        ...
        unsigned long long* pTimes = sampledTimes.get_data_ptr();

        unsigned long long e1, e2, n;
        for ( e2=0; e2<E2; e2++ )
        {
            for ( e1=0; e1<E1; e1++ )
            {
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
detectSampledRegionE1E2(const hoNDArray<T>& data, unsigned long long& startE1, unsigned long long& endE1, unsigned long long& startE2, unsigned long long& endE2)
{
    try
    {
        hoNDArray<unsigned long long> sampledTimes;
        GADGET_CHECK_RETURN_FALSE(detectSampledTimesE1E2(data, sampledTimes));

        unsigned long long E1 = sampledTimes.get_size(0);
        unsigned long long E2 = sampledTimes.get_size(1);
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
copyAlongE1(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startE1, unsigned long long endE1)
{
    try
    {
        unsigned long long NDim = src.get_number_of_dimensions();
        GADGET_CHECK_RETURN_FALSE(NDim>=2);

        unsigned long long RO = dst.get_size(0);
        unsigned long long E1 = dst.get_size(1);

        unsigned long long RO_src = src.get_size(0);
        unsigned long long E1_src = src.get_size(1);
        ...
        unsigned long long N = dst.get_number_of_elements()/(RO*E1);

        unsigned long long n, e1;
        for ( n=0; n<N; n++ )
        {
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
copyAlongROE1(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startRO, unsigned long long endRO, unsigned long long startE1, unsigned long long endE1)
{
    try
    {
        unsigned long long NDim = src.get_number_of_dimensions();
        ...
        for ( n=0; n<(int)N; n++ )
        {
            for ( unsigned long long e1=startE1; e1<=endE1; e1++ )
            {
                unsigned long long offset = n*RO*E1+e1*RO+startRO;
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
copyAlongROE1E2(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startRO, unsigned long long endRO, unsigned long long startE1, unsigned long long endE1, unsigned long long startE2, unsigned long long endE2)
{
    try
    {
        unsigned long long NDim = src.get_number_of_dimensions();
        ...
        unsigned long long RO = dst.get_size(0);
        unsigned long long E1 = dst.get_size(1);
        unsigned long long E2 = dst.get_size(2);

        unsigned long long RO_src = src.get_size(0);
        unsigned long long E1_src = src.get_size(1);
        unsigned long long E2_src = src.get_size(2);
        ...
        unsigned long long N = dst.get_number_of_elements()/(RO*E1*E2);
        const T* pSrc = src.begin();
        T* pDst = dst.begin();
        ...
        for ( n=0; n<(int)N; n++ )
        {
            for ( unsigned long long e2=startE2; e2<=endE2; e2++ )
            {
                for ( unsigned long long e1=startE1; e1<=endE1; e1++ )
                {
                    unsigned long long offset = n*RO*E1*E2+e2*E1*RO+e1*RO+startRO;
                    memcpy(pDst+offset, pSrc+offset, sizeof(T)*(endRO-startRO+1));
                }
            }
        }
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
copyAlongROE1TransitionBand(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startRO, unsigned long long endRO, 
        unsigned long long startE1, unsigned long long endE1, unsigned long long transBandRO, unsigned long long transBandE1)
{
    try
    {
        unsigned long long NDim = src.get_number_of_dimensions();
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
copyAlongROE1E2TransitionBand(const hoNDArray<T>& src, hoNDArray<T>& dst, unsigned long long startRO, unsigned long long endRO, 
                        unsigned long long startE1, unsigned long long endE1, unsigned long long startE2, unsigned long long endE2, 
                        unsigned long long transBandRO, unsigned long long transBandE1, unsigned long long transBandE2)
{
    try
    {
        unsigned long long NDim = src.get_number_of_dimensions();
        ...
        else
        {
            ...
            unsigned long long Nxyz = RO*E1*E2;
            for ( ii=0; ii<Nxyz; ii++ )
            {
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::findDimIndex(const std::vector<DimensionRecordType>& dimStartingIndexes, ISMRMRDDIM dim, unsigned long long ind)
{
    unsigned long long N = dimStartingIndexes.size();
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
extractSubArrayForDim(const hoNDArray<T>& x, hoNDArray<T>& r, ISMRMRDDIM& dim, unsigned long long value, bool lessEqual)
{
    try
    {
        ...
        std::vector<unsigned long long> crop_offset(10, 0);
        crop_offset[0] = 0;
        crop_offset[1] = 0;
        crop_offset[2] = 0;
        crop_offset[3] = 0;
        ...
        std::vector<unsigned long long> crop_size(10, 0);
        crop_size[0] = (*dimX)[0];
        crop_size[1] = (*dimX)[1];
        crop_size[2] = (*dimX)[2];
        crop_size[3] = (*dimX)[3];
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
extractSubArrayForDim(const hoNDArray<T>& x, hoNDArray<T>& r, ISMRMRDDIM& dim1, unsigned long long value1, ISMRMRDDIM& dim2, unsigned long long value2, bool lessEqual)
{
    try
    {
        boost::shared_ptr< std::vector<unsigned long long> > dimX = x.get_dimensions();
...
template <typename T> 
bool gtPlusISMRMRDReconUtil<T>::
extractSubArrayForDim1LessEqualDim2Equal(const hoNDArray<T>& x, hoNDArray<T>& r, ISMRMRDDIM& dim1, unsigned long long value1, ISMRMRDDIM& dim2, unsigned long long value2)
{
    try
    {
        boost::shared_ptr< std::vector<unsigned long long> > dimX = x.get_dimensions();
...
template <typename T> 
int gtPlusISMRMRDReconUtil<T>::addPrePostZeros(unsigned long long centre_column, unsigned long long samples)
{
    // 1 : pre zeros
    // 2 : post zeros
    // 0 : no zeros
...
template <typename T> 
void gtPlusISMRMRDReconUtil<T>::findStartEndRO(unsigned long long centre_column, unsigned long long samples, int& startRO, int& endRO)
{
...
template <typename T> 
void gtPlusISMRMRDReconUtil<T>::findStartEndROAfterZeroFilling(unsigned long long centre_column, unsigned long long samples_zerofilled, int& startRO, int& endRO)
{
    unsigned long long num = samples_zerofilled/2;
...
computeNoisePrewhiteningMatrix(const hoNDArray<T>& noise, double noiseBandWidth, double receiverBWRatio, double ADCSamplingTimeinSecond, hoMatrix<T>& prewhiteningMatrix)
{
    try
    {
        unsigned long long RO = noise.get_size(0);
        unsigned long long E1 = noise.get_size(1);
        unsigned long long CHA = noise.get_size(2);
...
performNoisePrewhitening(hoNDArray<T>& data, const hoMatrix<T>& prewhiteningMatrix)
{
    try
    {
        ...
        unsigned long long N = data.get_number_of_elements()/(RO*E1*CHA);
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
zpadResize2D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, hoNDArray<T>& dataResized)
{
    try
    {
        unsigned long long RO = data.get_size(0);
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
zpadResize2DOnKSpace(const hoNDArray<T>& kspace, unsigned long long sizeX, unsigned long long sizeY, hoNDArray<T>& dataResized)
{
    try
    {
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
zpadResize3D(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataResized)
{
    try
    {
        unsigned long long RO = data.get_size(0);
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
zpadResize3DOnKSpace(const hoNDArray<T>& kspace, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, hoNDArray<T>& dataResized)
{
    try
    {
        unsigned long long RO = kspace.get_size(0);
        unsigned long long E1 = kspace.get_size(1);
        unsigned long long E2 = kspace.get_size(2);
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
zpadResize2DFilter(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, const hoNDArray<T>& filter2D, hoNDArray<T>& dataResized)
{
    try
    {
        unsigned long long RO = data.get_size(0);
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
zpadResize3DFilter(const hoNDArray<T>& data, unsigned long long sizeX, unsigned long long sizeY, unsigned long long sizeZ, const hoNDArray<T>& filter3D, hoNDArray<T>& dataResized)
{
    try
    {
        unsigned long long RO = data.get_size(0);
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
coilMap2DNIHInner(const hoNDArray<T>& data, hoNDArray<T>& coilMap, unsigned long long ks, unsigned long long power)
{
    try
    {
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
coilMap2DNIH(const hoNDArray<T>& data, hoNDArray<T>& coilMap, ISMRMRDCOILMAPALGO algo, unsigned long long ks, unsigned long long power, unsigned long long iterNum, typename realType<T>::Type thres)
{
    try
    {
        typedef typename realType<T>::Type value_type;
        ...
        unsigned long long N = data.get_number_of_elements()/(RO*E1*CHA);
        unsigned long long num = RO*E1*CHA;
...
template <typename T> 
bool gtPlusISMRMRDReconUtilComplex<T>::
coilMap3DNIH(const hoNDArray<T>& data, hoNDArray<T>& coilMap, ISMRMRDCOILMAPALGO algo, unsigned long long ks, unsigned long long power, unsigned long long iterNum, typename realType<T>::Type thres)
{
    try
    {
        typedef typename realType<T>::Type value_type;
        ...
        unsigned long long N = data.get_number_of_elements()/(RO*E1*E2*CHA);
...
coilCombine(const hoNDArray<T>& data, const hoNDArray<T>& coilMap, hoNDArray<T>& combined)
{
    ...
    {
        unsigned long long NDim = data.get_number_of_dimensions();
        unsigned long long NDimCoil = coilMap.get_number_of_dimensions();
        ...
        boost::shared_ptr< std::vector<unsigned long long> > dim = data.get_dimensions();
        boost::shared_ptr< std::vector<unsigned long long> > dimCoil = coilMap.get_dimensions();

        unsigned long long N = coilMap.get_number_of_elements();
        unsigned long long num = data.get_number_of_elements()/coilMap.get_number_of_elements();

        std::vector<unsigned long long> dimCombined(*dim);
        dimCombined.erase(dimCombined.begin()+2);
        combined.create(&dimCombined);

        std::vector<unsigned long long> dimCombinedCurr(*dimCoil);
        dimCombinedCurr[2] = 1;

        unsigned long long NCombined = combined.get_number_of_elements()/num;
...
conjugateSymmetry2D(const hoNDArray<T>& kspace, hoNDArray<T>& kspaceConj)
{
    try
    {
        ...
        unsigned long long centerRO = RO/2;
        unsigned long long centerE1 = E1/2;
...
conjugateSymmetry3D(const hoNDArray<T>& kspace, hoNDArray<T>& kspaceConj)
{
    try
    {
        ...
        unsigned long long centerRO = RO/2;
        unsigned long long centerE1 = E1/2;
        unsigned long long centerE2 = E2/2;
