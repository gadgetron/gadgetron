...
public:
    ...
    bool appy_KLT_coil_compression_coeff_3D(const hoNDArray<T>& data, const std::vector< hoNDArray<T> >& coeff, hoNDArray<T>& dataEigen);
                ...
                {
                    {
                        ...
                        this->appy_KLT_coil_compression_coeff_3D(workOrder3DT->data_, upstreamCoilCoeffData, data_dst_);
                        if ( performTiming_ ) { gt_timer3_.stop(); }
                    ...
                    {
                        this->appy_KLT_coil_compression_coeff_3D(workOrder3DT->ref_, upstreamCoilCoeffRef, ref_dst_);
...

                    {
                        ...
                        this->appy_KLT_coil_compression_coeff_3D(refRecon, upstreamCoilCoeffRefRecon, refRecon_upstream);
...

                    {
                        ...
                        this->appy_KLT_coil_compression_coeff_3D(refCoilMap, upstreamCoilCoeffRefRecon, refCoilMap_upstream);
...
bool gtPlusReconWorker3DT<T>::coilCompression(WorkOrderType* workOrder3DT)
{
    ...
    try
    {
            ...
            && workOrder3DT->recon_algorithm_!=ISMRMRD_L1SPIRIT_SLEP_MOTION_COMP )
        {
            ...
            if ( N < dataN )
            {
                std::vector<hoNDArray<T> > coilCompressionCoef(dataN);
                for ( n=0; n<N; n++ )
                {
...
template <typename T>
bool gtPlusReconWorker3DT<T>::appy_KLT_coil_compression_coeff_3D(const hoNDArray<T>& data, const std::vector< hoNDArray<T> >& coeff, hoNDArray<T>& dataEigen)
{
    try
    {
        ...
        if (LastDimData>1)
        {
            ...
            for (n = 0; n<LastDimData; n++)
            {
                ...
                hoNDKLT<T> klt;
                klt.prepare(coeff[n]);
                klt.transform(dataLastDim, dataEigenLastDim, 3);
    ...
    {
        GERROR_STREAM("Errors in gtPlusReconWorker3DT<T>::appy_KLT_coil_compression_coeff_3D(std::vector<hoNDArray<T> >& coeff) ... ");
        return false;
    }
...
bool gtPlusReconWorker3DT<T>::performPartialFourierFilter(gtPlusReconWorkOrder3DT<T>& workOrder3DT, hoNDArray<T>& kspace)
{
    try
    {
        ...
        {
            Gadgetron::apply_kspace_filter_ROE1E2(kspace, workOrder3DT.filterROE1E2_partialfourier_, buffer3DT_partial_fourier);
        ...
        {
            Gadgetron::apply_kspace_filter_ROE1E2(kspace, workOrder3DT.filterRO_partialfourier_,
        ...
        else
        {
            ...
            if ( workOrder3DT.filterRO_partialfourier_.get_number_of_elements() == RO )
            {
                ...
                Gadgetron::apply_kspace_filter_RO(*pSrc, workOrder3DT.filterRO_partialfourier_, *pDst);
                std::swap(pSrc, pDst);
            ...
            if ( workOrder3DT.filterE1_partialfourier_.get_number_of_elements() == E1 )
            {
                ...
                Gadgetron::apply_kspace_filter_E1(*pSrc, workOrder3DT.filterE1_partialfourier_, *pDst);
                std::swap(pSrc, pDst);
                filterPerformed = true;
            }
            ...
            if ( workOrder3DT.filterE2_partialfourier_.get_number_of_elements() == E2 )
            {
                ...
                Gadgetron::apply_kspace_filter_E2(*pSrc, workOrder3DT.filterE2_partialfourier_, *pDst);
                std::swap(pSrc, pDst);
                filterPerformed = true;
            }
...
bool gtPlusReconWorker3DT<T>::performPartialFourierPOCSRecon(WorkOrderType& workOrder3DT, hoNDArray<T>& kspace)
{
    try
    {
        ...
        else
        {
            ...
            Gadgetron::generate_symmetric_filter_ref(RO, workOrder3DT.start_RO_, workOrder3DT.end_RO_, filterRO);
        ...
        else
        {
            ...
            Gadgetron::generate_symmetric_filter_ref(E1, workOrder3DT.start_E1_, workOrder3DT.end_E1_, filterE1);
        ...
        if ( (workOrder3DT.start_E2_<0 || workOrder3DT.end_E2_<0) )
        {
            ...
            Gadgetron::generate_symmetric_filter_ref(E2, 0, E2 - 1, filterE2);
        }
        else
        {
            ...
            Gadgetron::generate_symmetric_filter_ref(E2, workOrder3DT.start_E2_, workOrder3DT.end_E2_, filterE2);
        ...
        // kspace filter
        Gadgetron::apply_kspace_filter_ROE1E2(kspaceIter, filterRO, filterE1, filterE2, buffer3DT_partial_fourier);
