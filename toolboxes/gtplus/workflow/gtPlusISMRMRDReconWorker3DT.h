...
public:
    ...
    virtual bool prepRef(WorkOrderType* workOrder3DT, 
                        ...
                        unsigned long long dataE1, 
                        unsigned long long dataE2);
    ...
    virtual bool estimateJobSize(gtPlusReconWorkOrder<T>* workOrder3DT, unsigned long long maxNumOfBytesPerJob, unsigned long long overlapBetweenJobs, unsigned long long numOfNodes, unsigned long long& jobSize);
...
protected:
    ...
    unsigned long long startE2_;
    unsigned long long endE2_;
};
...
estimateCoilMap(gtPlusReconWorkOrder3DT<T>* workOrder3DT, const hoNDArray<T>& ref_src, const hoNDArray<T>& ref_dst, const hoNDArray<T>& ref_coil_map_dst)
{
    try
    {
        ...
        unsigned long long dstCHA = ref_coil_map_dst.get_size(3);

        bool same_combinationcoeff_allN = false;
        unsigned long long whichN_combinationcoeff = 0;
        ...
        if ( !reconKSpace )
        {
                ...
                || (workOrder3DT->coilMap_->get_size(2)!=E2) )
            {
                ...
                {
                    unsigned long long usedN = whichN_combinationcoeff;
                    ...
                    GADGET_CHECK_RETURN_FALSE(gtPlusISMRMRDReconUtilComplex<T>().coilMap3DNIH(buffer3DT, 
                            ...
                            workOrder3DT->csm_powermethod_num_, workOrder3DT->csm_iter_num_, workOrder3DT->csm_iter_thres_));
                ...
                else
                {
                    ...
                    GADGET_CHECK_RETURN_FALSE(gtPlusISMRMRDReconUtilComplex<T>().coilMap3DNIH(buffer3DT, 
                            *workOrder3DT->coilMap_, workOrder3DT->coil_map_algorithm_, 
                            ...
                            workOrder3DT->csm_iter_num_, workOrder3DT->csm_iter_thres_));
                    GADGET_CHECK_PERFORM(performTiming_, gt_timer3_.stop());
                }
            }
...
template <typename T> 
bool gtPlusReconWorker3DT<T>::
performCalibImpl(const hoNDArray<T>& , const hoNDArray<T>& , WorkOrderType* , unsigned long long )
{
    return true;
}
                                            ...
                                            int startE2, int endE2)
{
    try
    {
        ...
        unsigned long long E2 = ref.get_size(2);
...
bool gtPlusReconWorker3DT<T>::prepRefByAveragingCrossN(WorkOrderType* workOrder3DT, const hoNDArray<T>& ref, bool averageAllRef, int numOfModes, hoNDArray<T>& refRecon)
{
    try
    {
        ...
        unsigned long long E1 = ref.get_size(1);
        unsigned long long E2 = ref.get_size(2);
        unsigned long long CHA = ref.get_size(3);
        unsigned long long N = ref.get_size(4);
        ...
        else if ( averageAllRef && (numOfModes>=1) && (numOfModes<=N-1) )
        {
            ...
            hoMatrix<T> A(N, RO*E1*E2*CHA, const_cast<T*>(ref.begin()));
            hoMatrix<T> A_KLF(N, RO*E1*E2*CHA, refKLF.begin());
        ...
        else if ( !averageAllRef && (numOfModes>=1) && (numOfModes<=N-1) )
        {
            ...
            hoMatrix<T> A(N, RO*E1*E2*CHA, const_cast<T*>(ref.begin()));
            hoMatrix<T> A_KLF(N, RO*E1*E2*CHA, refRecon.begin());
            GADGET_CHECK_RETURN_FALSE(gtPlus_util_.computeKLFilter(A, numOfModes, A_KLF));
        }
        else
        {
...
bool gtPlusReconWorker3DT<T>::prepRef(WorkOrderType* workOrder3DT, const hoNDArray<T>& ref, 
                                ...
                                int startE1, int endE1, 
                                int startE2, int endE2, 
                                unsigned long long dataE1, unsigned long long dataE2)
{
    try
    {
        unsigned long long dataRO = workOrder3DT->data_.get_size(0);
        unsigned long long dataN = workOrder3DT->data_.get_size(4);

        unsigned long long RO = ref.get_size(0);
        unsigned long long E1 = ref.get_size(1);
        unsigned long long E2 = ref.get_size(2);
        unsigned long long srcCHA = ref.get_size(3);
        unsigned long long N = ref.get_size(4);
...
bool gtPlusReconWorker3DT<T>::coilCompression(WorkOrderType* workOrder3DT)
{
    ...
    try
    {
        unsigned long long RO = workOrder3DT->ref_recon_.get_size(0);
        unsigned long long E1 = workOrder3DT->ref_recon_.get_size(1);
        unsigned long long E2 = workOrder3DT->ref_recon_.get_size(2);
        unsigned long long srcCHA = workOrder3DT->ref_recon_.get_size(3);
        unsigned long long N = workOrder3DT->ref_recon_.get_size(4);
        ...
        if ( workOrder3DT->coil_compression_ && workOrder3DT->recon_algorithm_!=ISMRMRD_SPIRIT && workOrder3DT->recon_algorithm_!=ISMRMRD_L1SPIRIT )
        {
            ...
            if ( workOrder3DT->coilCompressionCoef_->size()!=dataN )
            {
                ...
                {
                    std::vector<unsigned long long> allNDim(4);
                    allNDim[0] = RO;
                    allNDim[1] = E1;
                    allNDim[2] = E2;
                    allNDim[3] = srcCHA;
...
bool gtPlusReconWorker3DT<T>::unmixCoeff(const hoNDArray<T>& kerIm, const hoNDArray<T>& coilMap, hoNDArray<T>& unmixCoeff, hoNDArray<T>& gFactor)
{
    try
    {
        ...
        unsigned long long E2 = kerIm.get_size(2);
        unsigned long long srcCHA = kerIm.get_size(3);
        unsigned long long dstCHA = kerIm.get_size(4);
...
bool gtPlusReconWorker3DT<T>::applyImageDomainKernelImage(const hoNDArray<T>& aliasedIm, const hoNDArray<T>& kerIm, hoNDArray<T>& kerImBuffer, hoNDArray<T>& complexIm)
{
    try
    {
        ...
        std::vector<unsigned long long> dimIm4D(4);
        dimIm4D[0] = RO;
        dimIm4D[1] = E1;
        dimIm4D[2] = E2;
        dimIm4D[3] = dstCHA;

        unsigned long long num = aliasedIm.get_number_of_elements()/ (RO*E1*E2*srcCHA);
...
bool gtPlusReconWorker3DT<T>::afterUnwrapping(WorkOrderType* workOrder3DT)
{
    try
    {
        ...
        unsigned long long whichN_coilmap = 0;
            ...
            if ( averageallN_coilmap )
            {
                ...
                else
                {
                    ...
                    if ( N > 1 )
                    {
                        ...
                        GADGET_CHECK_RETURN_FALSE(gtPlusISMRMRDReconUtilComplex<T>().coilMap3DNIH(aveComplexIm, *workOrder3DT->coilMap_, workOrder3DT->coil_map_algorithm_, workOrder3DT->csm_kSize_, workOrder3DT->csm_powermethod_num_, workOrder3DT->csm_iter_num_, workOrder3DT->csm_iter_thres_));
                        GADGET_CHECK_PERFORM(performTiming_, gt_timer2_.stop());
                        GADGET_EXPORT_ARRAY_COMPLEX(debugFolder_, gt_exporter_, *workOrder3DT->coilMap_, "coilMap");
                    }
                    else
                    {
                        GADGET_CHECK_PERFORM(performTiming_, gt_timer2_.start("full res coil map : compute 3D coil map ...  "));
                        GADGET_CHECK_RETURN_FALSE(gtPlusISMRMRDReconUtilComplex<T>().coilMap3DNIH(buffer3DT, *workOrder3DT->coilMap_, workOrder3DT->coil_map_algorithm_, workOrder3DT->csm_kSize_, workOrder3DT->csm_powermethod_num_, workOrder3DT->csm_iter_num_, workOrder3DT->csm_iter_thres_));
                        GADGET_CHECK_PERFORM(performTiming_, gt_timer2_.stop());
                        GADGET_EXPORT_ARRAY_COMPLEX(debugFolder_, gt_exporter_, *workOrder3DT->coilMap_, "coilMap");
                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ( same_coilmap_allN )
                    {
                        ...
                        GADGET_CHECK_RETURN_FALSE(gtPlusISMRMRDReconUtilComplex<T>().coilMap3DNIH(complexImN, coilMapN, workOrder3DT->coil_map_algorithm_, workOrder3DT->csm_kSize_, workOrder3DT->csm_powermethod_num_, workOrder3DT->csm_iter_num_, workOrder3DT->csm_iter_thres_));
        ...
        unsigned long long imRO = workOrder3DT->complexIm_.get_size(0);
        unsigned long long imE1 = workOrder3DT->complexIm_.get_size(1);
        unsigned long long imE2 = workOrder3DT->complexIm_.get_size(2);
        unsigned long long imCHA = workOrder3DT->complexIm_.get_size(3);
        ...
        unsigned long long mid_RO = imRO/2;
        unsigned long long mid_E1 = imE1/2;
        unsigned long long mid_E2 = imE2/2;

        unsigned long long n, cha;
...
bool gtPlusReconWorker3DT<T>::performPartialFourierPOCSRecon(WorkOrderType& workOrder3DT, hoNDArray<T>& kspace)
{
    try
    {
        ...
        unsigned long long E1 = kspace.get_size(1);
        unsigned long long E2 = kspace.get_size(2);
        unsigned long long CHA = kspace.get_size(3);
        unsigned long long N = kspace.get_size(4);
...
bool gtPlusReconWorker3DT<T>::performPartialFourierFengHuangRecon(WorkOrderType& workOrder3DT, hoNDArray<T>& kspace)
{
    try
    {
        ...
        unsigned long long startSymE2, endSymE2;
...
bool gtPlusReconWorker3DT<T>::calibFengHuang(WorkOrderType& workOrder3DT, const hoNDArray<T>& src, const hoNDArray<T>& dst, ho6DArray<T>& kernel)
{
    try
    {
        ...
        unsigned long long E2 = src.get_size(2);
        unsigned long long srcCHA = src.get_size(3);
        unsigned long long N = src.get_size(4);

        unsigned long long kx = workOrder3DT.partialFourier_FengHuang_kSize_RO_;
        unsigned long long ky = workOrder3DT.partialFourier_FengHuang_kSize_E1_;
        unsigned long long kz = workOrder3DT.partialFourier_FengHuang_kSize_E2_;
        ...
        #endif
        {
           /* hoNDArrayMemoryManaged<T> A_mem(colA, rowA, gtPlus_mem_manager_);
            ...
            hoNDArray<T> B_mem(colB, rowB);
            hoNDArray<T> K_mem(colB, colA);
            ...
            for ( ii=0; ii<num; ii ++ )
            {
                ...
                unsigned long long ro, e1, e2, row(0);
                int x, y, z;
                                                ...
                                                int startE2, int endE2, ho6DArray<T>& kernel)
{
    try
    {
        ...
        unsigned long long kz = kernel.get_size(2);

        int halfKx = kx/2;
        int halfKy = ky/2;
        int halfKz = kz/2;

        unsigned long long kerN = kernel.get_size(5);
        ...
        #endif
        {
            hoNDArrayMemoryManaged<T> D_mem(colD, rowD, gtPlus_mem_manager_);
            ...
            for ( ii=0; ii<num; ii ++ )
            {
                ...
                unsigned long long colCenter = colD/2;
...
template <typename T> 
bool gtPlusReconWorker3DT<T>::
estimateJobSize(gtPlusReconWorkOrder<T>* workOrder3DT, unsigned long long maxNumOfBytesPerJob, unsigned long long overlapBetweenJobs, unsigned long long numOfNodes, unsigned long long& jobSize)
{
    try
    {
        ...
        unsigned long long totalJobNum = RO;
        jobSize = std::ceil( (double)(totalJobNum+overlapBetweenJobs*(nodeN-1))/(double)nodeN );

        unsigned long long numOfBytesPerJob = sizeof(T)*( E1*E2*srcCHA*dstCHA*jobSize + 2*E1*E2*srcCHA*jobSize );
