...
struct DimensionRecordCompare
{
    ...
    bool operator()(const std::pair<ISMRMRDDIM, unsigned long long>& a, const std::pair<ISMRMRDDIM, unsigned long long>& b) const
    {
...
public:
    ...
    bool ismrmrdDimIndex9D(std::vector<unsigned long long>& ind, const ISMRMRDDIM& dim, unsigned long long value);

    // find the permute order for ISMRMRD
    bool findISMRMRDPermuteOrder(const std::vector<ISMRMRDDIM>& dimsSrc, const std::vector<ISMRMRDDIM>& dimsDst, std::vector<unsigned long long>& order);

    // print the dimension names
    std::string printISMRMRDDimensions(const std::vector<ISMRMRDDIM>& dims);

    // print the dimension size
    std::string printISMRMRDDimensionSize(const std::vector<unsigned long long>& sizes);
    ...
    // ----------------------------------
    unsigned long long reconSizeRO_;
    unsigned long long reconSizeE1_;
    unsigned long long reconSizeE2_;
...
ismrmrdDimIndex9D(std::vector<unsigned long long>& ind, const ISMRMRDDIM& dim, unsigned long long value)
{
    GADGET_CHECK_RETURN_FALSE(ind.size()>(unsigned long long)(dim-DIM_ReadOut));
...
findISMRMRDPermuteOrder(const std::vector<ISMRMRDDIM>& dimsSrc, const std::vector<ISMRMRDDIM>& dimsDst, std::vector<unsigned long long>& order)
{
    GADGET_CHECK_RETURN_FALSE(dimsSrc.size()==dimsDst.size());
    order.resize(dimsSrc.size());

    unsigned long long NDim = dimsSrc.size();
...
template <typename T> 
std::string gtPlusISMRMRDReconWorkFlow<T>::
printISMRMRDDimensionSize(const std::vector<unsigned long long>& sizes)
{
    using namespace std;

    if ( sizes.empty() ) return std::string("[ ]");

    unsigned long long NDim = sizes.size();
