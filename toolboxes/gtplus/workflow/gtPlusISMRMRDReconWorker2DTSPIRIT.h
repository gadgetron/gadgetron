...
performCalibPrep(const hoNDArray<T>& ref_src, const hoNDArray<T>& ref_dst, gtPlusReconWorkOrder2DT<T>* workOrder2DT)
{
    try
    {
        ...
        unsigned long long dstCHA = ref_dst.get_size(2);

        unsigned long long kRO = workOrder2DT->spirit_kSize_RO_;
        unsigned long long kE1 = workOrder2DT->spirit_kSize_E1_;
...
template <typename T> 
bool gtPlusReconWorker2DTSPIRIT<T>::
performCalibImpl(const hoNDArray<T>& ref_src, const hoNDArray<T>& ref_dst, gtPlusReconWorkOrder2DT<T>* workOrder2DT, unsigned long long n, unsigned long long usedS)
{
    try
    {
        unsigned long long RO = workOrder2DT->data_.get_size(0);
...
performUnwrapping(gtPlusReconWorkOrder2DT<T>* workOrder2DT, const hoNDArray<T>& data_dst)
{
    try
    {
        ...
        unsigned long long refN = workOrder2DT->kernelIm_->get_size(4);

        unsigned long long usedS;
        ...
        unsigned long long jobN = workOrder2DT->job_num_of_N_;
        unsigned long long jobMegaBytes = workOrder2DT->job_max_Megabytes_;
        unsigned long long overlapN = workOrder2DT->job_overlap_;
        ...
        if ( !splitJobs )
        {
            if ( jobMegaBytes>0 )
            {
                unsigned long long jobN = jobMegaBytes/(RO*E1*srcCHA*dstCHA*sizeof(T)/1024/1024);
        ...
        if ( splitJobs )
        {
            ...
            if ( runJobsOnCloud )
            {
                ...
                //unsigned long long numOfBytesPerJob = sizeof(T)*( RO*E1*srcCHA*dstCHA*jobN + 2*RO*E1*srcCHA*jobN );
