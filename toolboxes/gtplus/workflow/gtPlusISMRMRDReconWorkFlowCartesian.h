...
convertToReconSpace2D(hoNDArray<T>& input_, hoNDArray<T>& output_, bool isKSpace)
{
    try
    {
        unsigned long long RO = res_.get_size(0);
        unsigned long long E1 = res_.get_size(1);
        ...
        {
            unsigned long long encodingE1 = reconSizeE1_;
...
convertToReconSpace3D(hoNDArray<T>& input_, hoNDArray<T>& output_, bool isKSpace)
{
    try
    {
        unsigned long long RO = res_.get_size(0);
        unsigned long long E1 = res_.get_size(1);
        unsigned long long E2 = res_.get_size(2);
        ...
        else if ( (encodingFOV_E1_>=reconFOV_E1_) && (encodingFOV_E2_>=reconFOV_E2_) )
        {
            ...
            unsigned long long encodingE2 = reconSizeE2_;
...
postProcessing()
{
    try
    {
        ...
        unsigned long long RO = res_.get_size(0);
        unsigned long long E1 = res_.get_size(1);
        unsigned long long E2 = res_.get_size(4);
        ...
        if ( E2_.second > 1 )
        {
            ...
            //unsigned long long NDim = dataCurr_.get_number_of_dimensions();
            //std::vector<unsigned long long> order(NDim, 1);
...
configureWorkOrder(const std::vector<ISMRMRDDIM>& dims)
{
    try
    {
        ...
        std::vector<unsigned long long> dimSize(dims.size());
        std::vector<unsigned long long> dimSizeRef(dims.size(), 1);
        unsigned long long indChannelDim = 2;
        ...
        std::vector<unsigned long long> dimReconSize(5);
        ...
        std::vector<unsigned long long> dimReconSizeRef(5);
        ...
        unsigned long long N2D = dimReconSize[0]*dimReconSize[1];
        unsigned long long N2DRef = dimReconSizeRef[0]*dimReconSizeRef[1];

        unsigned long long N3D = N2D*dimReconSize[2];
        unsigned long long N3DRef = N2DRef*dimReconSizeRef[2];

        // allocate the results
        unsigned long long num_channels_res = workOrder_->num_channels_res_;

        std::vector<unsigned long long> dimResSize(dimSize);
        ...
        // call up the recon
        unsigned long long dim8, dim7, dim6, dim5, dim4, dim3, dim2;
        for ( dim8=0; dim8<dimSize[8]; dim8++ )
        {
            for ( dim7=0; dim7<dimSize[7]; dim7++ )
            {
                for ( dim6=0; dim6<dimSize[6]; dim6++ )
                {
                    ...
                    {
                        std::vector<unsigned long long> ind(10, 0);
                        ...
                        std::vector<unsigned long long> indWorkOrder(5, 0);
                        ...
                        {
                            std::vector<unsigned long long> indRef(10, 0);
                            ...
                            std::vector<unsigned long long> indRefWorkOrder(10, 0);
                            for ( dim4=0; dim4<dimSize[4]; dim4++ )
                            {
                                unsigned long long dim4_ref = dim4;
                                ...
                                for ( dim3=0; dim3<dimSize[3]; dim3++ )
                                {
                                    unsigned long long dim3_ref = dim3;
                                    ...
                                    else
                                    {
                                        for ( dim2=0; dim2<dimSize[2]; dim2++ )
                                        {
                                            unsigned long long dim2_ref = dim2;
                        ...
                        // copy the results
                        std::vector<unsigned long long> indRes(ind);
                        indRes[0] = 0;
                        indRes[1] = 0;
                        indRes[2] = 0;
                        indRes[3] = 0;
