...
struct gtPlusMemoryObj
{
    ...
    unsigned long long chunk_id_;

    // starting address of this memory
    void* mem_ptr_;

    // memory size in bytes
    unsigned long long len_bytes_;
};
...
struct gtPlusMemoryChunkObj
{
    ...
    // memory size in bytes
    unsigned long long len_chunk_bytes_;
};
...
public:
    ...
    gtPlusMemoryManager(unsigned long long aligned_bytes=4, unsigned long long preallocated_bytes=4294967296);
    ...
    virtual bool increase(unsigned long long added_bytes);
    ...
    unsigned long long totalFreeMemory() const;

    // maximal free memory chunk
    unsigned long long maxFreeMemoryChunkSize() const;
...
protected:
    ...
    // aligned bytes
    unsigned long long aligned_bytes_;

    // make sure the allocate and free are thread-safe
    boost::mutex mutex_;

    // allocate memory
    void* allocateImpl(unsigned long long size);
