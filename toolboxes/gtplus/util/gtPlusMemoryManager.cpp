...
gtPlusMemoryManager::gtPlusMemoryManager(unsigned long long aligned_bytes, unsigned long long preallocated_bytes) : aligned_bytes_(aligned_bytes)
{
    try
    {
...
gtPlusMemoryManager::~gtPlusMemoryManager()
{
    try
    {
        ...
        unsigned long long num = memory_.size();
...
void* gtPlusMemoryManager::allocate(unsigned long long size)
{
    void* ptr = NULL;
    mutex_.lock();
...
void* gtPlusMemoryManager::allocateImpl(unsigned long long size)
{
    try
    {
        if ( size == 0 )
    ...
    {
        GADGET_ERROR_MSG("Error happened in gtPlusMemoryManager::allocate(unsigned long long size) : " << size);
        return NULL;
    }
...
void gtPlusMemoryManager::defragmentImpl()
{
    try
    {
        ...
        unsigned long long N = free_list_.size();
        ...
        unsigned long long numChunk = memory_.size();

        unsigned long long n, jj;
...
void* gtPlusMemoryManager::allocateChunkAsUsed(unsigned long long chunkId)
{
...
bool gtPlusMemoryManager::increase(unsigned long long added_bytes)
{
    try
    {
    ...
    {
        GADGET_ERROR_MSG("Errors happened in gtPlusMemoryManager::increase(unsigned long long added_bytes) : " << added_bytes/1024/1024 << " MegaBytes ...");
        return false;
    }
...
void gtPlusMemoryManager::_allocate_memory( unsigned long long size, void*& data )
{
    #ifdef USE_MKL
...
unsigned long long gtPlusMemoryManager::totalFreeMemory() const
{
...
unsigned long long gtPlusMemoryManager::maxFreeMemoryChunkSize() const
{
    unsigned long long maxChunkSize = 0;
    MemoryListType::const_iterator iter = free_list_.begin();
    for ( ; iter!=free_list_.end(); iter++ )
    {
