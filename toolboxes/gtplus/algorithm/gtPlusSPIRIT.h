...
calib(const ho3DArray<T>& acsSrc, const ho3DArray<T>& acsDst, double thres, int kRO, int kE1, int a, int b, ho6DArray<T>& ker)
{
    try
    {
        ...
        unsigned long long RO = acsSrc.get_size(0);
        unsigned long long E1 = acsSrc.get_size(1);
        unsigned long long srcCHA = acsSrc.get_size(2);
        unsigned long long dstCHA = acsDst.get_size(2);
        ...
        unsigned long long sRO = kROhalf;
        unsigned long long eRO = RO - kROhalf -1;

        unsigned long long sE1 = kE1half;
        unsigned long long eE1 = E1 - kE1half -1;

        unsigned long long lenRO = eRO-sRO+1;
        unsigned long long lenE1 = eE1-sE1+1;

        unsigned long long rowA = lenE1*lenRO;
        unsigned long long colA = (kRO*kE1-1)*srcCHA;
        unsigned long long colB = dstCHA;
        ...
        for ( int e1=(int)sE1; e1<=(int)eE1; e1++ )
        {
            ...
            for ( int ro=sRO; ro<=(int)eRO; ro++ )
            {
                ...
                for ( unsigned long long src=0; src<srcCHA; src++ )
                {
            ...
            int kRO, int kE1, int kE2, int a, int b, int c, hoNDArray<T>& ker)
{
    try
    {
        ...
        unsigned long long RO = acsSrc.get_size(0);
        unsigned long long E1 = acsSrc.get_size(1);
        unsigned long long E2 = acsSrc.get_size(2);
        unsigned long long srcCHA = acsSrc.get_size(3);
        unsigned long long dstCHA = acsDst.get_size(3);
        ...
        unsigned long long sE2 = kE2half;
        unsigned long long eE2 = E2 - kE2half -1;
        unsigned long long lenE2 = eE2-sE2+1;

        unsigned long long colA = (kRO*kE1*kE2-1)*srcCHA;
        if ( overDetermineRatio > 1.0 )
        {
            unsigned long long maxRowA = std::ceil(overDetermineRatio*colA);
            unsigned long long maxROUsed = maxRowA/(lenE1*lenE2);
            if ( maxROUsed < lenRO )
            {
        ...
        unsigned long long rowA = lenRO*lenE1*lenE2;
        unsigned long long colB = dstCHA;
        ...
        std::vector<unsigned long long> kerInd(8);
        kerInd[7] = 0;
        kerInd[6] = 0;
        kerInd[5] = 0;
                ...
                for ( int ke1=-kE1half; ke1<=kE1half; ke1++ ) 
                {
                    ...
                    for ( int kro=-kROhalf; kro<=kROhalf; kro++ ) 
                    {
                        ...
                        if ( kro!=0 || ke1!=0 || ke2!=0 )
                        {
                            ...
                            {
                                kerInd[4] = dst;
                                unsigned long long offset = ker.calculate_offset(kerInd);
...
kspaceDomainConvKernel3D(const hoNDArray<T>& ker, int kRO, int kE1, int kE2, int a, int b, int c, ho5DArray<T>& convKerFlip, bool minusI, bool ROis3rdDim)
{
    try
    {
        ...
        std::vector<unsigned long long> kerInd(8), convKerInd(8);
                ...
                for ( kro=-kROhalf; kro<=kROhalf; kro++ )
                {
                    ...
                    for ( dst=0; dst<dstCHA; dst++ )
                    {
                        ...
                        for ( src=0; src<srcCHA; src++ )
                        {
                            ...
                            unsigned long long offsetKer = ker.calculate_offset(kerInd);
                            unsigned long long offsetConvKer = convKer.calculate_offset(convKerInd);
...
imageDomainKernelRO3D(const hoNDArray<T>& ker, int kRO, int kE1, int kE2, int a, int b, int c, int ro, hoNDArray<T>& kImRO, bool minusI)
{
    try
    {
        ...
        // allocate image domain kernel
        unsigned long long kConvE1 = convKerFlip.get_size(1);
        unsigned long long kConvE2 = convKerFlip.get_size(2);
...
imageDomainKernelE1E2RO(const hoNDArray<T>& kImRO, int e1, int e2, hoNDArray<T>& kImE1E2RO)
{
    try
    {
        boost::shared_ptr< std::vector<unsigned long long> > dim = kImRO.get_dimensions();

        std::vector<unsigned long long> dimR(*dim);
        dimR[0] = e1;
        dimR[1] = e2;
...
bool gtPlusSPIRIT<T>::AdjointForwardKernel(const hoNDArray<T>& kImS2D, const hoNDArray<T>& kImD2S, hoNDArray<T>& kIm)
{
    try
    {
        boost::shared_ptr< std::vector<unsigned long long> > dimS2D = kImS2D.get_dimensions();

        unsigned long long NDim = kImS2D.get_number_of_dimensions();
        ...
        std::vector<unsigned long long> dimRes(*dimS2D);
        dimRes[NDim-2] = dstCHA;

        kIm.create(&dimRes);
        Gadgetron::clear(&kIm);

        unsigned long long N = kImS2D.get_number_of_elements()/srcCHA/dstCHA;
        ...
        #endif
        {
            ...
            for ( d=0; d<dstCHA; d++ )
            {
                for ( unsigned long long dprime=0; dprime<dstCHA; dprime++ )
                {
                    hoNDArray<T> dKer(N, kIm.begin()+d*N+dprime*N*dstCHA);

                    for ( unsigned long long s=0; s<srcCHA; s++ )
                    {
