...
public:
    ...
    unsigned long long numOfWavLevels_;
...
divideWavCoeffByNorm(hoNDArray<T>& wavCoeff, const hoNDArray<T>& wavCoeffNorm, T mu, T p, bool processApproxCoeff)
{
    try
    {
        ...
        unsigned long long W = wavCoeff.get_size(2);
        unsigned long long CHA = wavCoeff.get_size(3);
        ...
        else
        {
            ...
            unsigned long long num = wavCoeff.get_number_of_elements()/(RO*E1*W*CHA);
