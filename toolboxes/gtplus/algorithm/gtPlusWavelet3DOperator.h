...
public:
    ...
    bool dwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level);

    // perform the redundant haar wavelet inverse transform
    // in : [RO E1 E2 1+7*level], out : [RO E1 E2]
    bool idwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level);
...
forwardOperator(const hoNDArray<T>& x, hoNDArray<T>& y)
{
    try
    {
        boost::shared_ptr< std::vector<unsigned long long> > dims = x.get_dimensions();
        unsigned long long NDim = dims->size();

        unsigned long long RO = (*dims)[0];
        unsigned long long E1 = (*dims)[1];
        unsigned long long CHA = (*dims)[2];
        unsigned long long E2 = (*dims)[3];
        unsigned long long W = 1+7*numOfWavLevels_;
        ...
        unsigned long long num = x.get_number_of_elements()/(RO*E1*E2*CHA);
...
adjointOperator(const hoNDArray<T>& x, hoNDArray<T>& y)
{
    try
    {
        ...
        unsigned long long RO = (*dims)[0];
        unsigned long long E1 = (*dims)[1];
        unsigned long long E2 = (*dims)[2];
        unsigned long long W = (*dims)[3];
        unsigned long long CHA = (*dims)[4];
        ...
        unsigned long long num = x.get_number_of_elements()/(RO*E1*E2*W*CHA);
...
L1Norm(const hoNDArray<T>& wavCoeff, hoNDArray<T>& wavCoeffNorm)
{
    try
    {
        boost::shared_ptr< std::vector<unsigned long long> > dims = wavCoeff.get_dimensions();

        std::vector<unsigned long long> dimR(*dims);
        dimR[4] = 1;
...
divideWavCoeffByNorm(hoNDArray<T>& wavCoeff, const hoNDArray<T>& wavCoeffNorm, T mu, T p, bool processApproxCoeff)
{
    try
    {
        unsigned long long RO = wavCoeff.get_size(0);
        unsigned long long E1 = wavCoeff.get_size(1);
        unsigned long long E2 = wavCoeff.get_size(2);
        unsigned long long W = wavCoeff.get_size(3);
        unsigned long long CHA = wavCoeff.get_size(4);
        ...
        else
        {
            ...
            unsigned long long num = wavCoeff.get_number_of_elements()/(RO*E1*E2*W*CHA);
...
template <typename T> 
bool gtPlusWavelet3DOperator<T>::
dwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level)
{
    try
    {
        unsigned long long RO = in.get_size(0);
        unsigned long long E1 = in.get_size(1);
        unsigned long long E2 = in.get_size(2);
        ...
        for (unsigned long long n=0; n<level; n++)
        {
            T* lll = pOut;
    ...
    {
        GADGET_ERROR_MSG("Errors in gtPlusWavelet3DOperator<T>::dwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level) ... ");
        return false;
    }
    return true;
}
...
template <typename T> 
bool gtPlusWavelet3DOperator<T>::
idwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level)
{
    try
    {
        unsigned long long RO = in.get_size(0);
    ...
    {
        GADGET_ERROR_MSG("Errors in gtPlusWavelet3DOperator<T>::idwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level) ... ");
        return false;
    }
    return true;
}
...
gradTask(const hoNDArray<T>& x, hoNDArray<T>& g)
{
    try
    {
        ...
        unsigned long long RO = complexIm_.get_size(0);
        unsigned long long E1 = complexIm_.get_size(1);
        unsigned long long CHA = complexIm_.get_size(2);
        unsigned long long E2 = complexIm_.get_size(3);
...
firstDimensionScale(hoNDArray<T>& wavCoeff, T& scaleFactor)
{
    try
    {
        ...
        unsigned long long num = wavCoeff.get_number_of_elements()/(RO*E1*E2*W);
        ...
        for ( ii=0; ii<num; ii++ )
        {
            for ( unsigned long long n=0; n<numOfWavLevels_; n++ )
            {
                // 2, 3
