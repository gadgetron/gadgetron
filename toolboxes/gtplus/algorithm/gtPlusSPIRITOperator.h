...
setForwardKernel(boost::shared_ptr< hoNDArray<T> >& forward_kernel, bool computeAdjForwardKernel)
{
    try
    {
        ...
        boost::shared_ptr< std::vector<unsigned long long> > dims = forward_kernel->get_dimensions();
        unsigned long long NDim = dims->size();

        std::vector<unsigned long long> dimSrc(NDim-1), dimDst(NDim-1);
...
setAdjointForwardKernel(boost::shared_ptr< hoNDArray<T> >& adjoint_forward_kernel)
{
    try
    {
        ...
        // allocate the helper memory
        boost::shared_ptr< std::vector<unsigned long long> > dims = adjoint_forward_kernel_->get_dimensions();
        unsigned long long NDim = dims->size();
