...
public:
    ...
    bool kerPattern(std::vector<int>& kE1, std::vector<int>& oE1, int accelFactor, unsigned long long kNE1, bool fitItself);
...
template <typename T> 
bool gtPlusGRAPPA<T>::
kerPattern(std::vector<int>& kE1, std::vector<int>& oE1, int accelFactor, unsigned long long kNE1, bool fitItself)
{
    if ( accelFactor == 1 )
    {
        kE1.resize(1, 0);
    ...
    int kRO, const std::vector<int>& kE1, const std::vector<int>& oE1, ho5DArray<T>& ker)
{
    try
    {
        ...
        unsigned long long kNE1 = kE1.size();
        ...
        unsigned long long sE1 = std::abs(kE1[0]);
        unsigned long long eE1 = E1 -1 - kE1[kNE1-1];

        unsigned long long lenRO = eRO-kROhalf+1;

        unsigned long long rowA = (eE1-sE1+1)*lenRO;
        unsigned long long colA = kRO*kNE1*srcCHA;
        unsigned long long colB = dstCHA*oNE1;
        ...
        for ( e1=(int)sE1; e1<=(int)eE1; e1++ )
        {
            for ( int ro=kROhalf; ro<=(int)eRO; ro++ )
            {
                ...
                unsigned long long src, dst, ke1, oe1;
                int kro;
            ...
            ho7DArray<T>& ker)
{
    try
    {
        ...
        unsigned long long kNE2 = kE2.size();
        unsigned long long oNE2 = oE2.size();
        ...
        unsigned long long sE2 = std::abs(kE2[0]);
        unsigned long long eE2 = E2 -1 - kE2[kNE2-1];

        unsigned long long lenRO = eRO-kROhalf+1;
        unsigned long long lenE1 = eE1-sE1+1;

        unsigned long long rowA = (eE2-sE2+1)*lenE1*lenRO;
        unsigned long long colA = kRO*kNE1*kNE2*srcCHA;
        unsigned long long colB = dstCHA*oNE1*oNE2;
        ...
        for ( e2=(int)sE2; e2<=(int)eE2; e2++ )
        {
            ...
            for ( e1=(int)sE1; e1<=(int)eE1; e1++ )
            {
                for ( int ro=kROhalf; ro<=(int)eRO; ro++ )
                {
                    ...
                    unsigned long long src, dst, ke1, ke2, oe1, oe2;
                    int kro;
...
imageDomainKernel3D(const ho7DArray<T>& ker, int kRO, const std::vector<int>& kE1, const std::vector<int>& kE2, const std::vector<int>& oE1, const std::vector<int>& oE2, int ro, int e1, int e2, hoNDArray<T>& kIm)
{
    try
    {
        ...
        if ( kIm.get_number_of_elements() < (unsigned long long)ro*e1*e2*srcCHA*dstCHA )
        {
