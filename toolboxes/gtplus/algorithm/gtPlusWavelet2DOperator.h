...
forwardOperator(const hoNDArray<T>& x, hoNDArray<T>& y)
{
    try
    {
        ...
        unsigned long long W = 1+3*numOfWavLevels_;
...
adjointOperator(const hoNDArray<T>& x, hoNDArray<T>& y)
{
    try
    {
        ...
        unsigned long long num = x.get_number_of_elements()/(RO*E1*W);
...
dwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level)
{
    ...
    {
        GADGET_ERROR_MSG("Errors in gtPlusWavelet2DOperator<T>::dwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level) ... ");
        return false;
    }
    return true;
}
...
idwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level)
{
    try
    {
        ...
        for (n=level-1; n>=0; n--)
        {
            ...
            for (e1=0; e1<E1; e1++)
            {
                unsigned long long ii = e1*RO+RO-1;
            ...
            for (ro=0; ro<RO; ro++)
            {
                unsigned long long ii = (E1-1)*RO+ro;
                T vLL = pOut[ii];
                T vLH = pTmp [ii];
    ...
    {
        GADGET_ERROR_MSG("Errors in gtPlusWavelet2DOperator<T>::idwtRedundantHaar(const hoNDArray<T>& in, hoNDArray<T>& out, unsigned long long level) ... ");
        return false;
    }
    return true;
}
