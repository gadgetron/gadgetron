...
namespace Gadgetron{
  ...
  cuNDFFT<T>::fft_int( cuNDArray< complext<T> > *input, std::vector<unsigned long long> *dims_to_transform, int direction, bool do_scale )
  {
    std::vector<unsigned long long> new_dim_order;
    std::vector<unsigned long long> reverse_dim_order;
    std::vector<int> dims;
    std::vector<unsigned long long> dim_count(input->get_number_of_dimensions(),0);
    
    unsigned int array_ndim = input->get_number_of_dimensions();
    boost::shared_ptr< std::vector<unsigned long long> > array_dims = input->get_dimensions();
    ...
    reverse_dim_order = std::vector<unsigned long long>(array_ndim,0);
    for (unsigned int i = 0; i < array_ndim; i++) {
  ...
  template<class T> void
  cuNDFFT<T>::fft( cuNDArray< complext<T> > *input, std::vector<unsigned long long> *dims_to_transform )
  {
    fft_int(input, dims_to_transform, CUFFT_FORWARD, false);
  }
  
  template<class T> void
  cuNDFFT<T>::ifft( cuNDArray< complext<T> > *input, std::vector<unsigned long long> *dims_to_transform, bool do_scale )
  {
  ...
  {
    std::vector<unsigned long long> dims(1,dim_to_transform);
  ...
  {
    std::vector<unsigned long long> dims(input->get_number_of_dimensions(),0);
