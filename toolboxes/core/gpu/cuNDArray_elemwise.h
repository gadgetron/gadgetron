 ...
 */
template <class T>
EXPORTGPUCORE boost::shared_ptr<cuNDArray<typename realType<T>::Type>> abs_square(const cuNDArray<T>* x);
...
/**
 * @brief Create a new array of the complex conjugate of the input array. For real arrays a copy of the input array is
 * return.
 * @param[in] x Input array.
 ...
 */
template <class T>
EXPORTGPUCORE boost::shared_ptr<cuNDArray<T>> real_to_complex(const cuNDArray<typename realType<T>::Type>* x);
...
template <class T>
EXPORTGPUCORE boost::shared_ptr<cuNDArray<T>> imag_to_complex(const cuNDArray<typename realType<T>::Type>* x);
 ...
 */
template <class T>
EXPORTGPUCORE void clamp(cuNDArray<T>* x, typename realType<T>::Type min, typename realType<T>::Type max, T min_val,
                         T max_val);
 ...
 */
template <class T>
EXPORTGPUCORE void clamp(cuNDArray<T>* x, typename realType<T>::Type min, typename realType<T>::Type max);
 ...
 */
template <class T>
EXPORTGPUCORE void normalize(cuNDArray<T>* x, typename realType<T>::Type val = typename realType<T>::Type(1));
 ...
 */
template <class T>
EXPORTGPUCORE void shrink1(cuNDArray<T>* x, typename realType<T>::Type gamma, cuNDArray<T>* out = 0x0);
 ...
 */
template <class T>
EXPORTGPUCORE void pshrink(cuNDArray<T>* x, typename realType<T>::Type gamma, typename realType<T>::Type p,
                           cuNDArray<T>* out = 0x0);
 ...
 */
template <class T>
EXPORTGPUCORE void shrinkd(cuNDArray<T>* x, cuNDArray<typename realType<T>::Type>* s, typename realType<T>::Type gamma,
                           cuNDArray<T>* out = 0x0);

/**
 * @brief In place p-shrinkage (soft thresholding, multi-dimensional), i.e. pshrink(x,s,gamma,p) =
 * x/s*max(s-gamma*s^(p-1),0).
 * @param[out] out Output array. Can be 0x0 in which case an in place transform is performed.
 * @param[in,out] x Input array (and output array if out == 0x0).
 * @param[in] gamma Shrinkage control parameter
 * @param[in] p p value of the shrinkage. Should be less than 1 and more than 0.
 */
template <class T>
EXPORTGPUCORE void pshrinkd(cuNDArray<T>* x, cuNDArray<typename realType<T>::Type>* s, typename realType<T>::Type gamma,
                            typename realType<T>::Type p, cuNDArray<T>* out = 0x0);
} // namespace Gadgetron
