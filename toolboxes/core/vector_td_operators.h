...
namespace Gadgetron{
  ...
  template<> struct vectorTDReturnType<unsigned long long, int> {typedef int type; };
  ...
  template<> struct vectorTDReturnType<float, unsigned long long> {typedef float type; };
  template<> struct vectorTDReturnType<unsigned long long, float> {typedef float type; };
  ...
  template<> struct vectorTDReturnType<unsigned long long, double> {typedef double type; };
  template<> struct vectorTDReturnType<double, unsigned long long> {typedef double type; };
  ...
  { 
    for(unsigned long long i=0; i<D; i++ ) v1.vec[i] += v2.vec[i];
  }

  template< class T, unsigned long long D > __inline__ __host__ __device__ 
  void operator+= ( vector_td<T,D> &v1, const T &v2 )
  {
    for(unsigned long long i=0; i<D; i++ ) v1.vec[i] += v2;
  }
  ...
  template< class T, class R, unsigned long long D > __inline__ __host__ __device__ 
  ...
  { 
    ...
    for(unsigned long long i=0; i<D; i++ ) res.vec[i] = v1.vec[i]+v2.vec[i];
  ...
  {
    vector_td<typename vectorTDReturnType<T,R>::type,D> res;
    for(unsigned long long i=0; i<D; i++ ) res.vec[i] =v1.vec[i]+v2;
    return res;
  }
  ...
  {
    vector_td<T,D> res;
    for(unsigned long long i=0; i<D; i++ ) res.vec[i] = -v1.vec[i];
    return res;
  }
  ...
  vector_td<typename vectorTDReturnType<T,R>::type,D> operator/ ( const vector_td<T,D> &v1, const vector_td<R,D> &v2 )
  {
    ...
    for(unsigned long long i=0; i<D; i++ ) res[i] /= v2[i];
    return res;
  }
  ...
  { 
    for(unsigned long long i=0; i<D; i++ ) if(!(v1.vec[i] == v2.vec[i])) return false;
    return true;
  }
  ...
  bool operator!= ( const vector_td<T,D> &v1, const vector_td<T,D> &v2 ) 
  { 
    for(unsigned long long i=0; i<D; i++ ) if((v1.vec[i] != v2.vec[i])) return true;
    return false;
  }
  ...
  template< class T, unsigned long long D > __inline__ __host__ __device__ 
  void operator<<= ( vector_td<T,D> &v1, unsigned long long shifts ) 
  { 
    for(unsigned long long i=0; i<D; i++ ) v1.vec[i] <<= shifts;
  }
  ...
  template< class T, unsigned long long D > __inline__ __host__ __device__ 
  vector_td<T,D> operator<< ( const vector_td<T,D> &v1, unsigned long long shifts ) 
  { 
