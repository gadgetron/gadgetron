...
namespace Gadgetron{
  /** 
      ...
      and memory allocation routines of FFFTW are NOT 
      threadsafe.

      Access it for instance with

      hoFFT<float>::instance()
  */
  template <typename T> class EXPORTCPUCORE hoFFT
  {

  public:
    static hoFFT<T>* instance(); 
  ...
  protected:
    ...
    hoFFT(){ set_function_pointers(); }
    virtual ~hoFFT() { fftw_cleanup_ptr_(); }
	...
	if (fft_storage == 0){
	  std::cout << "Failed to allocate buffer for hoFFT" << std::endl;
	  return;
	}
	...
	if (fft_plan == 0){
	  ...
	  std::cout << "Failed to create plan for hoFFT" << std::endl;
	  return;
	}
	mutex_.unlock();
      }
    ...
    static hoFFT<T>* instance_;
    boost::mutex mutex_;
  };

  template<> inline void hoFFT<float>::set_function_pointers()
  {
  ...
  template<> inline void hoFFT<double>::set_function_pointers()
  {
