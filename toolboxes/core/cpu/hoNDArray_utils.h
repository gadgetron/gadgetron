  ...
  permute( hoNDArray<T> *in, hoNDArray<T> *out, std::vector<size_t> *dim_order, int shift_mode = 0) 
  {
    ...
    if (stride == 1) {
        ...
        if ((num_permute_task==1) || (num_dim_task==0)) {
            ...
            std::vector<size_t> dim_permute(nDim-num_dim_task);
            for (size_t i = num_dim_task; i < nDim; i++) {
                dim_permute[i - num_dim_task] = in->get_size(i);
            }

            size_t stride = 1;
            for (size_t i = 0; i < num_dim_task; i++) {
                stride *= in->get_size(i);
            }
...
#pragma omp parallel default(none) shared(stride, num_permute_task, num_dim_task, dim_permute, dim_order_int, nDim, in, out, o) private(n)
#endif USE_OMP
            {
                ...
                std::vector<size_t> ind_permute_in(dim_permute.size(), 0), ind_in(nDim, 0), ind_permute_out(nDim, 0), ind_out(nDim, 0);
...
#pragma omp for
                ...
                for (n = 0; n < num_permute_task; n++) {
                    ...
                    memcpy(&ind_out[0] + num_dim_task, &ind_permute_in[0], sizeof(size_t)*ind_permute_in.size());

                    for (size_t p = 0; p < stride; p++)
                    {
                        in->calculate_index(p, ind_permute_out);
                        memcpy(&ind_in[0], &ind_permute_out[0], sizeof(size_t)*num_dim_task);
                        ...
                        o[offset_out] = in->get_data_ptr()[n*stride+p];
                    }
                }
            }
        }
