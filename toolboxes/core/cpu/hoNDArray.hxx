...
template <typename T> 
hoNDArray<T>::hoNDArray(boost::shared_ptr< std::vector<size_t> > dimensions, 
...
template <typename T> 
inline void hoNDArray<T>::create(boost::shared_ptr< std::vector<size_t> > dimensions)
{
    this->create(dimensions.get());
}
...
bool hoNDArray<T>::deserialize(char* buf, size_t& len)
{
    ...
    {
        GADGET_ERROR_MSG("Errors happended in hoNDArray<T>::deserialize(char* buf, size_t len) ...");
        return false;
    }
...
//template class hoNDArray<size_t>;
