...
namespace Gadgetron{
    ...
    template <typename T> 
    inline size_t NDArray<T>::calculate_offset(const std::vector<long long>& ind) const
    {
    ...
    template <typename T> 
    inline T& NDArray<T>::operator()( const std::vector<long long>& ind )
    {
        size_t idx = this->calculate_offset(ind);
        GADGET_DEBUG_CHECK_THROW(idx < this->get_number_of_elements());
        return this->data_[idx];
    ...
    template <typename T> 
    inline const T& NDArray<T>::operator()( const std::vector<long long>& ind ) const
    {
        size_t idx = this->calculate_offset(ind);
        GADGET_DEBUG_CHECK_THROW(idx < this->get_number_of_elements());
        return this->data_[idx];
