...
NFFT_iterate_body( typename reald<REAL,D>::Type alpha, typename reald<REAL,D>::Type beta, REAL W, 
		   vector_td<size_t, D> matrix_size_os, unsigned int number_of_batches, complext<REAL> *image,
		   ...
		   vector_td<REAL,D> sample_position, vector_td<long long,D> grid_position )
{
  ...
  for( unsigned int batch=0; batch<number_of_batches; batch++ ){
    ...
    const complext<REAL> grid_value = 
      image[ (batch*gridDim.y+blockIdx.y)*prod(matrix_size_os) + co_to_idx<D>( *((vector_td<size_t, D>*)&grid_position), matrix_size_os ) ];
...
NFFT_iterate( typename reald<REAL,1>::Type alpha, typename reald<REAL,1>::Type beta, REAL W, 
	      vector_td<size_t,1> matrix_size_os, unsigned int number_of_batches, complext<REAL> *image,
...
NFFT_iterate( typename reald<REAL,2>::Type alpha, typename reald<REAL,2>::Type beta, REAL W, 
	      vector_td<size_t,2> matrix_size_os, unsigned int number_of_batches, complext<REAL> *image,
...
NFFT_iterate( typename reald<REAL,3>::Type alpha, typename reald<REAL,3>::Type beta, REAL W, 
	      vector_td<size_t,3> matrix_size_os, unsigned int number_of_batches, complext<REAL> *image,
...
NFFT_iterate( typename reald<REAL,4>::Type alpha, typename reald<REAL,4>::Type beta, REAL W, 
	      vector_td<size_t,4> matrix_size_os, unsigned int number_of_batches, complext<REAL> *image,
