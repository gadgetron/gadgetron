...
namespace Gadgetron{
  ...
  protected:
    ...
    public:
      ...
      virtual void initialize(boost::shared_ptr< std::vector<unsigned long long> > image_dims,
	...
	*this->b_k -= *this->d_k;
	...
	for (int i=0; i < reg_ops.size(); i++){
      ...
      virtual void initialize(boost::shared_ptr< std::vector<unsigned long long> > image_dimensions,
			      REAL normalization_factor = REAL(1))
      {
    ...
    protected:
      ...
      boost::shared_ptr< std::vector<unsigned long long> > image_dims;
    };
  ...
  protected:
	...
	throw std::runtime_error( "Error: sbSolver::validate_encoding_operator : operator not set" );
      ...
      boost::shared_ptr< std::vector<unsigned long long> > op_dims = op->get_domain_dimensions();
      if( op_dims->size() == 0 ){
    ...
    virtual void validate_regularization_operators( std::vector<unsigned long long> *image_dims )
    {
    ...
    virtual void validate_solver()
    {
      ...
      boost::shared_ptr< std::vector<unsigned long long> > op_dims = this->encoding_operator_->get_domain_dimensions();
