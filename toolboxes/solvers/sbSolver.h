...
protected:
	...
	public:
		...
		virtual boost::shared_ptr<std::vector<unsigned int> > get_codimensions(){
	...
	public:
		...
		virtual void update_encoding_space(ARRAY_TYPE_ELEMENT* encoding_space)
		{
			for (int i=0; i < reg_ops.size(); i++){
				ARRAY_TYPE_ELEMENT tmp(reg_ops[i]->get_codomain_dimensions(),encoding_space->get_data_ptr()+op_cont->get_offset(i));
				tmp = *d_ks[i];
				tmp -= *b_ks[i];
				if (this->prior.get())
					tmp += *p_Ms[i];
		...
		virtual void initialize()
		{
			...
			for (int i=0; i<reg_ops.size(); i++){
				d_ks[i] = boost::shared_ptr<ARRAY_TYPE_ELEMENT>(new ARRAY_TYPE_ELEMENT(reg_ops[i]->get_codomain_dimensions()));
				clear(d_ks[i].get());
				b_ks[i] = boost::shared_ptr<ARRAY_TYPE_ELEMENT>(new ARRAY_TYPE_ELEMENT(reg_ops[i]->get_codomain_dimensions()));
				clear(b_ks[i].get());
				if (this->prior.get()){
					p_Ms[i] = boost::shared_ptr<ARRAY_TYPE_ELEMENT>(new ARRAY_TYPE_ELEMENT(reg_ops[i]->get_codomain_dimensions()));
		...
		virtual void update_dk(ARRAY_TYPE_ELEMENT* u_k)
		{
			ARRAY_TYPE_REAL s_k(reg_ops.front()->get_codomain_dimensions());
	...
	public:
		...
		void set_image_dims(std::vector<unsigned int>* im_dims){
			image_dims = *im_dims;
	...
	virtual void initialize()
	{
		...
		if (non_negativity_filter_weight_ > REAL(0)){
			boost::shared_ptr<sbNonNegativityOperator> negOp(new sbNonNegativityOperator);
			negOp->set_weight(non_negativity_filter_weight_);
			negOp->set_image_dims(image_dims.get());
			regularization_operators_.push_back(negOp);
		}
		...
		for (int i=0; i < regularization_operators_.size(); i++){
			regularization_operators_[i]->initialize();
			...
			for( unsigned int inner_iteration=0; inner_iteration<inner_iterations; inner_iteration++ ) {
				...
				{ // Brackets used to free 'data' below as soon as it goes out of scope
					...
					for( unsigned int i=0; i< regularization_operators_.size(); i++ ){
						...
						tmp.create(op->get_codimensions().get(), data.get_data_ptr()+enc_op_container_->get_offset(i+1) );
