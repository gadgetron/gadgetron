/**
 * Implements the "Fast X-Ray CT Image Reconstruction Using a Linearized Augmented Lagrangian Method with Ordered
 * Subsets" method, by 	Hung Nien and Jeffrey A. Fessler
 */
#pragma once
#include "solver.h"
...
template <class ARRAY_TYPE> class osLALMSolver : public solver<ARRAY_TYPE, ARRAY_TYPE> {
  ...
  public:
    ...
    void set_regularization_iterations(int reg_it) { inner_iterations = reg_it; }

    void set_damping(REAL damp) { t = damp; }
    void set_alpha(REAL a) { alpha = a; }
    /**
     * Sets the preconditioning image. In most cases this is not needed, and the preconditioning is calculated based on
    ...
    boost::shared_ptr<ARRAY_TYPE> solve(ARRAY_TYPE* in) {
        ...
        std::vector<boost::shared_ptr<ARRAY_TYPE>> tmp_projections =
            this->encoding_operator_->projection_subsets(&tmp_projection);
        ...
        std::vector<int> isubsets(boost::counting_iterator<int>(0),
                                  boost::counting_iterator<int>(this->encoding_operator_->get_number_of_subsets()));
        ...
        for (int i = 0; i < _iterations; i++) {
            for (int isubset = 0; isubset < this->encoding_operator_->get_number_of_subsets(); isubset++) {
                ...
                *tmp_projections[subset] -= *subsets[subset];
                if (this->output_mode_ >= solver<ARRAY_TYPE, ARRAY_TYPE>::OUTPUT_VERBOSE) {
                    GINFO_STREAM("Iteration " << i << " Subset " << subset
                                              << " Update norm: " << nrm2(tmp_projections[subset].get()) << std::endl);
                }
    ...
    void set_encoding_operator(boost::shared_ptr<subsetOperator<ARRAY_TYPE>> encoding_operator) {
    ...
    virtual void set_tau(REAL tau) { tau0 = tau; }

  protected:
    /**
     * Solves an image denoising problem, using the regularization operators for denoising.
     * This is done via the AHMOD algorithm (see "A First-Order Primal-Dual Algorithm for Convex Problems with
     * Applications to Imaging" - Antonin Chambolle & Thomas Pock, 2010)
     * @param x
     * @param s
     * @param scaling
     */
    void denoise(ARRAY_TYPE& x, ARRAY_TYPE& s, ARRAY_TYPE& precon, REAL scaling, REAL avg_lambda) {
        ...
        for (auto it = 0u; it < inner_iterations; it++) {
            ...
            for (auto& reg_group : regularization_groups) {
                ...
                GINFO(std::string("Reg val: " + std::to_string(reg_val) + " Scaling " +
                                  std::to_string(scaling * avg_lambda) + "\n")
                          .c_str());
