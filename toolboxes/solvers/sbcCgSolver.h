...
template <class REAL, class ELEMENT_TYPE, class ARRAY_TYPE_REAL, class ARRAY_TYPE_ELEMENT, class INNER_SOLVER> class sbcCgSolver 
  : public sbCgSolver<REAL, ELEMENT_TYPE, ARRAY_TYPE_REAL, ARRAY_TYPE_ELEMENT, INNER_SOLVER>
{
public:

  sbcCgSolver() : sbCgSolver<REAL, ELEMENT_TYPE, ARRAY_TYPE_REAL, ARRAY_TYPE_ELEMENT, INNER_SOLVER>() 
  { 
  ...
  virtual boost::shared_ptr<ARRAY_TYPE_ELEMENT> solve( ARRAY_TYPE_ELEMENT *_f )
  {
    ...
    //
    if( !this->validate() ){
      this->solver_error( "sbcCgSolver::solve : setup failed validation");
    ...
    if( !this->initialize( d_k, b_k ) ){
      this->solver_error( "sbcCgSolver::solve : failed to initialize");
      return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
    }
    ...
    if( !f->get_data_ptr() ){
      this->solver_error( "sbcCgSolver::solve : memory allocation of f failed" );
      return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
    }
    ...
    if( !this->normalize( f, u_k, image_scale ) ){
      this->solver_error( "sbcCgSolver::solve : normalization failed" );
      return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
    }
    ...
    if( !f_k->get_data_ptr() ){
      this->solver_error( "sbcCgSolver::solve : memory allocation of f_k failed" );
      return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
    }
    ...
    if( !muEHf.get() || !muEHf->get_data_ptr() ){
      this->solver_error( "sbcCgSolver::solve : memory allocation of muEHf failed" );
      return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
    }
	...
	this->solver_error( "sbcCgSolver::solve : adjoint encoding operation failed on f_k" );
	return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
      }
	...
	this->solver_error( "sbcCgSolver::solve : error scaling EHf with mu" );
	return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
      }
      ...
      if( !core( this->tolerance_, this->inner_iterations_, 1, f_k, muEHf, u_k, d_k, b_k ) ){
      	this->solver_error( "sbcCgSolver::solve : core solver failed" );
      	return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
      } 
      ...
      if( encoded_image.create( f->get_dimensions().get() ) < 0 ){
        this->solver_error( "sbcCgSolver::solve : memory allocation for encoded image failed" );
        return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();      
      }

      if( this->encoding_operator_->mult_M( u_k.get(), &encoded_image ) < 0 ){
        this->solver_error( "sbcCgSolver::solve : computation of encoded image failed" );
        return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
      }

      if( !this->solver_axpy_element( ELEMENT_TYPE(0)-ELEMENT_TYPE(1), f.get(), &encoded_image )){ // notice the deliberate sign manipulation
        this->solver_error( "sbcCgSolver::solve : computation of update argument to f_k failed" );
        return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
      }
    ...
    if( !undo_normalization( u_k, 1/(image_scale) )){
      this->solver_error( "sbcCgSolver::solve : unable to undo normalization" );
      return boost::shared_ptr<ARRAY_TYPE_ELEMENT>();
    } 
