...
namespace Gadgetron
{
    ...
    // isEightConnected: if false, 4-connection is used
    template <typename T> EXPORTIMAGE
    ...
    EXPORTIMAGE void bwlabel_area_2d(const hoNDArray<unsigned int>& labelArray, std::vector<unsigned int>& labels, std::vector<unsigned int>& areas);

    // clean foreground and background using bwlabel
    template <typename T> EXPORTIMAGE
    void bwlabel_clean_fore_and_background(const hoNDArray<T>& input, T object_value, T bg_value, size_t obj_thres, size_t bg_size, bool isEightConnected, hoNDArray<T>& output);
}
