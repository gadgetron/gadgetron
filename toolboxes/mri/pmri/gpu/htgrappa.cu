		...
		std::list< unsigned int >* uncombined_channels)
{
	...
	std::vector<unsigned long long> rosTmp = *ref_data->get_dimensions();
  ...
  for (unsigned int i = 0; i < ros.size(); i++) {
    ...
    boost::shared_ptr< std::vector<unsigned long long> > dimTmp = ref_data->get_dimensions();
	...
	*/
	...
	std::vector<unsigned long long> ft_dims(2,0);ft_dims[1] = 1;
