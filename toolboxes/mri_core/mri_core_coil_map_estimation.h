...
namespace Gadgetron
{
    ...
    template<typename T> EXPORTMRICORE void coil_map_3d_Inati(const hoNDArray<T>& data, hoNDArray<T>& coilMap, size_t ks = 7, size_t kz = 5, size_t power = 3);

    // data: [RO E1 E2 CHA N S SLC ...], if E2==1, the 2D coil map estimation is assumed
    template<typename T> EXPORTMRICORE void coil_map_Inati(const hoNDArray<T>& data, hoNDArray<T>& coilMap, size_t ks = 7, size_t kz = 5, size_t power = 3);
    ...
    template<typename T> EXPORTMRICORE void coil_map_2d_Inati_Iter(const hoNDArray<T>& data, hoNDArray<T>& coilMap, size_t ks=7, size_t iterNum=5, typename realType<T>::Type thres=1e-3);

    // data: [RO E1 E2 CHA], true 3D coil map estimation
    template<typename T> EXPORTMRICORE void coil_map_3d_Inati_Iter(const hoNDArray<T>& data, hoNDArray<T>& coilMap, size_t ks=7, size_t kz=5, size_t iterNum=5, typename realType<T>::Type thres=0.001);
    ...
    // combined: [RO E1 E2 ...]
    template<typename T> EXPORTMRICORE void coil_combine(const hoNDArray<T>& data, const hoNDArray<T>& coilMap, size_t cha_dim, hoNDArray<T>& combined);
}
