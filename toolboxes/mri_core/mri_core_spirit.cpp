...
template <typename T>
void spirit2d_calib(const hoNDArray<T>& acsSrc, const hoNDArray<T>& acsDst,
                ...
                hoNDArray<T>& ker)
{
    try
    {
        GADGET_CHECK_THROW(acsSrc.dimensions_equal(&acsDst));
        GADGET_CHECK_THROW(acsSrc.get_size(0)>startRO + kRO);
        GADGET_CHECK_THROW(acsSrc.get_size(1)>startE1 + kE1);
        ...
        if (2 * kROhalf == kRO)
        {
            GWARN_STREAM("spirit2d_calib_convolution_kernel - 2*kROhalf == kRO " << kRO);
        }
        kRO = 2 * kROhalf + 1;
        ...
        if (2 * kE1half == kE1)
        {
            GWARN_STREAM("spirit2d_calib_convolution_kernel - 2*kE1half == kE1 " << kE1);
        }
        kE1 = 2 * kE1half + 1;
        ...
        if (2 * oROhalf == oRO)
        {
            GWARN_STREAM("spirit2d_calib_convolution_kernel - 2*oROhalf == oRO " << oRO);
        }
        oRO = 2 * oROhalf + 1;
        ...
        if (2 * oE1half == oE1)
        {
            GWARN_STREAM("spirit2d_calib_convolution_kernel - 2*oE1half == oE1 " << oE1);
        }
        oE1 = 2 * oE1half + 1;

        GADGET_CHECK_THROW(RO >= endRO);
        GADGET_CHECK_THROW(E1 >= endE1);

        // allocate kernel
        GADGET_CATCH_THROW(ker.create(kRO, kE1, srcCHA, dstCHA, oRO, oE1));
        ...
        size_t sE1 = startE1 + kE1half;
        size_t eE1 = endE1 - kE1half;
...
#pragma omp parallel default(none) shared(RO, E1, sRO, eRO, sE1, eE1, oRO, oE1, lenRO, lenE1, rowA, colA, colB, kRO, kE1, kROhalf, kE1half, oROhalf, oE1half, pAcsSrc, acsSrc, acsDst, srcCHA, dstCHA, thres, ker, std::cout) num_threads( (int)(oRO*oE1) ) if (oRO*oE1>=3)
        {
...
#pragma omp for
    ...
    {
        GADGET_THROW("Errors in spirit2d_calib(start, end) ... ");
    }

    return;
}

template EXPORTMRICORE void spirit2d_calib(const hoNDArray< std::complex<float> >& acsSrc, const hoNDArray< std::complex<float> >& acsDst, double thres, size_t kRO, size_t kE1, size_t oRO, size_t oE1, size_t startRO, size_t endRO, size_t startE1, size_t endE1, hoNDArray< std::complex<float> >& ker);
template EXPORTMRICORE void spirit2d_calib(const hoNDArray< std::complex<double> >& acsSrc, const hoNDArray< std::complex<double> >& acsDst, double thres, size_t kRO, size_t kE1, size_t oRO, size_t oE1, size_t startRO, size_t endRO, size_t startE1, size_t endE1, hoNDArray< std::complex<double> >& ker);

// ------------------------------------------------------------------------

template <typename T> 
void spirit2d_convert_to_convolution_kernel(const hoNDArray<T>& ker, size_t kRO, size_t kE1, size_t oRO, size_t oE1, hoNDArray<T>& convKer, bool minusI)
{
    try
    {
        ...
        GADGET_CHECK_THROW(oRO == ker.get_size(4));
        GADGET_CHECK_THROW(oE1 == ker.get_size(5));
        ...
        convKerBuf.create(convKRO, convKE1, srcCHA, dstCHA, oRO, oE1);
                ...
                for (ke1 = -kE1half; ke1 <= kE1half; ke1++)
                {
                    for (kro = -kROhalf; kro <= kROhalf; kro++)
                    {
                        ...
                        for (dst = 0; dst<dstCHA; dst++)
                        {
                            ...
                            {
                                convKerBuf(iro, ie1, src, dst, oro + oROhalf, oe1 + oE1half) = ker(kro + kROhalf, ke1 + kE1half, src, dst, oro + oROhalf, oe1 + oE1half);
                            }
                        }
                    }
                }
        ...
        GADGET_CATCH_THROW(Gadgetron::sum_over_dimension(convKerBuf, convKer2, 5));
        GADGET_CATCH_THROW(Gadgetron::sum_over_dimension(convKer2, conKerMean, 4));
        GADGET_CATCH_THROW(Gadgetron::scal((typename realType<T>::Type)(1.0 / (oRO*oE1)), conKerMean));
        ...
        for (ke1 = 0; ke1<convKE1; ke1++)
        {
            for (kro = 0; kro<convKRO; kro++)
            {
                for (dst = 0; dst<dstCHA; dst++)
                {
                    for (src = 0; src<srcCHA; src++)
                    {
                        convKer(kro, ke1, src, dst) = conKerMean(convKRO - 1 - kro, convKE1 - 1 - ke1, src, dst, 0, 0);
                    }
                }
            }
        }
        ...
        if (minusI)
        {
            for (dst = 0; dst<dstCHA; dst++)
            {
                T value = convKer(kRO - 1, kE1 - 1, dst, dst);
                convKer(kRO - 1, kE1 - 1, dst, dst) = value - T(1.0);
            }
        }
    }
    catch (...)
    {
        GADGET_THROW("Errors in spirit2d_convert_to_convolution_kernel(...) ... ");
    }

    return;
}

template EXPORTMRICORE void spirit2d_convert_to_convolution_kernel(const hoNDArray< std::complex<float> >& ker, size_t kRO, size_t kE1, size_t oRO, size_t oE1, hoNDArray< std::complex<float> >& convKer, bool minusI);
template EXPORTMRICORE void spirit2d_convert_to_convolution_kernel(const hoNDArray< std::complex<double> >& ker, size_t kRO, size_t kE1, size_t oRO, size_t oE1, hoNDArray< std::complex<double> >& convKer, bool minusI);
