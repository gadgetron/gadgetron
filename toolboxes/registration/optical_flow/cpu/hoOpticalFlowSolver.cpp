...
namespace Gadgetron{
  ...
  template<unsigned long long D> inline typename uintd<D>::Type 
  compute_stride( unsigned long long dim )
  {
  ...
  is_border_pixel_in_stride_dim_before( unsigned long long dim, typename uintd<D>::Type co, typename uintd<D>::Type dims )
  {
    if( co.vec[dim] == 0 )
      return true;
    else
  ...
  template<unsigned long long D> inline bool 
  is_border_pixel_in_stride_dim_after( unsigned long long dim, typename uintd<D>::Type co, typename uintd<D>::Type dims )
  {
  ...
  hoOpticalFlowSolver<T,D>::core_grad_spatial( T *fixed_image, T *moving_image, T *gradient_image, 
						  ...
						  unsigned long long num_batches_fixed, 
						  unsigned long long num_batches_moving )
  {        
    // Number of elements per partial derivate
    const unsigned long long num_elements_per_batch = prod(matrix_size);
    const unsigned long long num_elements_per_pdev_fixed = num_elements_per_batch*num_batches_fixed;
    const unsigned long long num_elements_per_pdev_moving = num_elements_per_batch*num_batches_moving;

    // Total number of elements for all partial derivatives
    const unsigned long long num_elements_total = std::max(num_elements_per_pdev_fixed, num_elements_per_pdev_moving)*D;
  
    for( unsigned long long idx = 0; idx<num_elements_total; idx++ ){
    
      // The (minimum) index in the slowest varying output dimension determines which partial derivative to compute 
      const unsigned long long stride_dim_fixed = idx/(num_elements_per_pdev_fixed);
      const unsigned long long stride_dim_moving = idx/(num_elements_per_pdev_moving);
      const unsigned long long stride_dim = std::min(stride_dim_fixed, stride_dim_moving);

      // Local index to the partial derivative
      const unsigned long long idx_in_pdev_fixed = idx-stride_dim_fixed*num_elements_per_pdev_fixed;
      const unsigned long long idx_in_pdev_moving = idx-stride_dim_moving*num_elements_per_pdev_moving;

      // Batch idx (second slowest varying dimension)   
      const unsigned long long batch_idx_fixed = idx_in_pdev_fixed/num_elements_per_batch;
      const unsigned long long batch_idx_moving = idx_in_pdev_moving/num_elements_per_batch;

      // Local index to the batch (should be identical for the fixed/moving image)
      const unsigned long long idx_in_batch = idx_in_pdev_moving-batch_idx_moving*num_elements_per_batch;
      ...
      const unsigned long long base_idx_moving = batch_idx_moving*num_elements_per_batch;
      const unsigned long long base_idx_fixed = batch_idx_fixed*num_elements_per_batch;

      unsigned long long stride_base_idx, fixed_idx, moving_idx;
  ...
  {        
    ...
    // Total number of elements for all partial derivatives
    const unsigned long long num_elements_total = std::max(num_elements_per_pdev_fixed, num_elements_per_pdev_moving);
    ...
    for( unsigned long long idx =0; idx < num_elements_total; idx++ ){
      ...
      // Ctr pixel
      const unsigned long long fixed_idx = idx_in_batch+base_idx_fixed;
      const unsigned long long moving_idx = idx_in_batch+base_idx_moving;
