...
namespace Gadgetron{
  ...
  template<class REAL, unsigned long long D> inline bool
  is_border_pixel( vector_td<unsigned long long,D> co, vector_td<unsigned long long,D> dims )
  {
  ...
  boost::shared_ptr< hoNDArray<REAL> > downsample( hoNDArray<REAL> *_in )
  {
    ...
    typename uintd<D>::Type matrix_size_in = from_std_vector<unsigned long long,D>( *_in->get_dimensions() );
    ...
    unsigned long long num_elements = prod(matrix_size_out);
    unsigned long long num_batches = 1;

    for( unsigned long long d=D; d<_in->get_number_of_dimensions(); d++ ){
...
#pragma omp parallel for
    ...
    for( int idx=0; idx < num_elements*num_batches; idx++ ){
      ...
      const unsigned long long frame_offset = idx/num_elements;
      ...
      const unsigned long long num_adds = 1 << D;

      unsigned long long actual_adds = 0;
      REAL res = REAL(0);

      for( unsigned long long i=0; i<num_adds; i++ ){
	...
	const unsigned long long in_idx = co_to_idx<D>(co_in+local_co, matrix_size_in)+frame_offset*prod(matrix_size_in);
	actual_adds++;
	res += in[in_idx];
      }    
  ...
  template<class REAL, unsigned long long D> boost::shared_ptr< hoNDArray<REAL> >
...
#pragma omp parallel for
    ...
    for( int idx=0; idx < num_elements*num_batches; idx++ ){
      ...
      const unsigned long long num_neighbors = 1 << D;
      const unsigned long long frame_idx = idx/num_elements;
