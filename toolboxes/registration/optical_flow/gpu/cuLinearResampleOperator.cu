...
cuLinearResampleOperator<REAL,T,D>::get_num_neighbors()
{
...
template<class REAL, class T, unsigned int D> __device__ 
T interpolate( unsigned int batch_no, 
	       typename reald<REAL,D>::Type co, 
...
template<class REAL, class T, unsigned int D> bool cuLinearResampleOperator<REAL,T,D>::
write_sort_arrays( thrust::device_vector<unsigned int> &sort_keys )
{
  ...
  if( !this->setup_grid( &blockDim, &gridDim, prod(matrix_size)*extended_dim )){
    std::cout << std::endl << "Error: cuResampleOperator :: write_sort_arrays : failed to determine grid setup." << std::endl;
    return false;
  }
  
  write_sort_arrays_kernel<REAL,D><<< gridDim, blockDim >>>
...
cuLinearResampleOperator<REAL,T,D>::mult_MH_M( cuNDArray<T> *in, cuNDArray<T> *out, bool accumulate )
{
  return -1;
}

// This macro is a workaround for Cudas missing support for pure virtual functions. 
// It defines mult_M (which is intended to be shared among all classes inherited from cuResampleOperator).
//
...
template class EXPORTGPUREG cuLinearResampleOperator<float,float,1>;
template class EXPORTGPUREG cuLinearResampleOperator<float,float_complext,1>;

template class EXPORTGPUREG cuLinearResampleOperator<float,float,2>;
template class EXPORTGPUREG cuLinearResampleOperator<float,float_complext,2>;

template class EXPORTGPUREG cuLinearResampleOperator<float,float,3>;
template class EXPORTGPUREG cuLinearResampleOperator<float,float_complext,3>;

template class EXPORTGPUREG cuLinearResampleOperator<float,float,4>;
template class EXPORTGPUREG cuLinearResampleOperator<float,float_complext,4>;

template class EXPORTGPUREG cuLinearResampleOperator<double,double,1>;
template class EXPORTGPUREG cuLinearResampleOperator<double,double_complext,1>;

template class EXPORTGPUREG cuLinearResampleOperator<double,double,2>;
template class EXPORTGPUREG cuLinearResampleOperator<double,double_complext,2>;

template class EXPORTGPUREG cuLinearResampleOperator<double,double,3>;
template class EXPORTGPUREG cuLinearResampleOperator<double,double_complext,3>;

template class EXPORTGPUREG cuLinearResampleOperator<double,double,4>;
template class EXPORTGPUREG cuLinearResampleOperator<double,double_complext,4>;
