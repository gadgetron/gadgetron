...
cuHSOpticalFlowSolver<REAL,D>::core_solver( cuNDArray<REAL> *gradient_image, cuNDArray<REAL> *stencil_image )
{
  ...
  if( !gradient_image ){
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : NULL input" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }

  if( gradient_image->get_number_of_dimensions() <= D ){
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : number of gradient image dimensions is too small" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }
  ...
  boost::shared_ptr< cuNDArray<REAL> > displacements_ping( new cuNDArray<REAL>());
  boost::shared_ptr< cuNDArray<REAL> > displacements_pong( new cuNDArray<REAL>());
  
  if( !displacements_ping->create( disp_dims.get() )) {
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : Unable to allocate storage for displacements (1)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }

  if( !displacements_pong->create( disp_dims.get() )) {
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : Unable to allocate storage for displacements (2)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }
  ...
  if( !cuNDA_clear(displacements_ping.get()) ){
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : Unable to clear displacement field (1)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }

  if( !cuNDA_clear(displacements_pong.get()) ){
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : Unable to clear displacement field (2)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }
  ...
  if( !this->setup_grid( &blockDim, &gridDim, number_of_elements, number_of_batches*D, true )){
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : Unable determine grid dimensions (1)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }
  ...
  if( cudaMalloc((void**)&continue_flag, sizeof(unsigned int) ) != cudaSuccess ) {
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : failed to allocate continuation flag" );
    return boost::shared_ptr< cuNDArray<REAL> >();    
  }
  ...
  while(true){
    ...
    if( cudaMemcpy( continue_flag, &_continue_flag, sizeof(unsigned int), cudaMemcpyHostToDevice ) != cudaSuccess ) {
      this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : failed to set continuation flag" );
      return boost::shared_ptr< cuNDArray<REAL> >();    
    }
    ...
    HornSchunk_kernel<REAL,D><<< gridDim, blockDim, (blockDim.x*blockDim.y)*sizeof(REAL) >>>
    ...
    if( cudaMemcpy(&_continue_flag, continue_flag, sizeof(unsigned int), cudaMemcpyDeviceToHost) != cudaSuccess ) {
      this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : failed to evaluate the continuation flag" );
      return boost::shared_ptr< cuNDArray<REAL> >();
    }
  ...
  if( cudaFree(continue_flag) != cudaSuccess ) {
    this->solver_error( "cuHSOpticalFlowSolver:: HornSchunk : failed to free continuation flag" );
    return boost::shared_ptr< cuNDArray<REAL> >();    
  }
