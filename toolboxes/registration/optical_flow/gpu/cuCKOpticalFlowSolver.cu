...
namespace Gadgetron{
  ...
  cuCKOpticalFlowSolver<REAL,D>::core_solver( cuNDArray<REAL> *gradient_image, cuNDArray<REAL> *stencil_image )
  {
    ...
    if( !gradient_image ){
      BOOST_THROW_EXCEPTION(runtime_error("cuCKOpticalFlowSolver::core_solver(): illegal input gradient image received."));
    }

    if( gradient_image->get_number_of_dimensions() <= D ){
      BOOST_THROW_EXCEPTION(runtime_error("cuCKOpticalFlowSolver::core_solver(): number of gradient image dimensions is too small."));
    }
    ...
    if( cudaMalloc((void**)&continue_flag, sizeof(unsigned int) ) != cudaSuccess ) {
      BOOST_THROW_EXCEPTION(runtime_error("cuCKOpticalFlowSolver::core_solver(): failed to allocate continuation flag."));
    }
	...
	BOOST_THROW_EXCEPTION(runtime_error("cuCKOpticalFlowSolver::core_solver(): failed to set continuation flag."));
      }
    ...
    if( cudaFree(continue_flag) != cudaSuccess ) {
      BOOST_THROW_EXCEPTION(runtime_error("cuCKOpticalFlowSolver::core_solver(): failed to free continuation flag."));
    }
