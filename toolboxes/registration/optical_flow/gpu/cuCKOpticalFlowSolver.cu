...
cuCKOpticalFlowSolver<REAL,D>::core_solver( cuNDArray<REAL> *gradient_image, cuNDArray<REAL> *stencil_image )
{
  ...
  if( !gradient_image ){
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : NULL input" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }

  if( gradient_image->get_number_of_dimensions() <= D ){
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : number of gradient image dimensions is too small" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }
  ...
  if( !displacements_ping->create( disp_dims.get() )) {
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : Unable to allocate storage for displacements (1)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }

  if( !displacements_pong->create( disp_dims.get() )) {
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : Unable to allocate storage for displacements (2)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }
  ...
  if( !cuNDA_clear(displacements_ping.get()) ){
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : Unable to clear displacement field (1)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }

  if( !cuNDA_clear(displacements_pong.get()) ){
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : Unable to clear displacement field (2)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }
  ...
  if( !this->setup_grid( &blockDim, &gridDim, number_of_elements, number_of_batches*(D+1), true, D+1 )){
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : Unable determine grid dimensions (1)" );
    return boost::shared_ptr< cuNDArray<REAL> >();
  }
  ...
  if( cudaMalloc((void**)&continue_flag, sizeof(unsigned int) ) != cudaSuccess ) {
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : failed to allocate continuation flag" );
    return boost::shared_ptr< cuNDArray<REAL> >();    
  }
  ...
  while(true){
    ...
    if( cudaMemcpy( continue_flag, &_continue_flag, sizeof(unsigned int), cudaMemcpyHostToDevice ) != cudaSuccess ) {
      this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : failed to set continuation flag" );
      return boost::shared_ptr< cuNDArray<REAL> >();    
    }
    ...
    if( cudaMemcpy(&_continue_flag, continue_flag, sizeof(unsigned int), cudaMemcpyDeviceToHost) != cudaSuccess ) {
      this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : failed to evaluate the continuation flag" );
      return boost::shared_ptr< cuNDArray<REAL> >();
    }
  ...
  if( cudaFree(continue_flag) != cudaSuccess ) {
    this->solver_error( "cuCKOpticalFlowSolver:: CorneliusKanade : failed to free continuation flag" );
    return boost::shared_ptr< cuNDArray<REAL> >();    
  }
