...
int main(int argc, char** argv)
{
  ...
  *recon_dims = to_std_vector(matrix_size); recon_dims->push_back(frames_per_reconstruction);
  ...
  FISTASolver<cuNDArray<_complext> > solver;
  ...
  if (w > 0.0){
  	boost::shared_ptr<cuHaarWaveletOperator<_complext,3> > wave(new cuHaarWaveletOperator<_complext,3>);
		...
		boost::shared_ptr<identityOperator<cuNDArray<_complext > > > I(new identityOperator<cuNDArray<_complext > >);
		I->set_domain_dimensions(recon_dims.get());
		I->set_codomain_dimensions(recon_dims.get());
		I->set_weight(w);
		solver.set_frame(wave);
  }
  solver.set_output_mode( FISTASolver<cuNDArray <_complext> >::OUTPUT_VERBOSE );
