...
int main(int argc, char** argv)
{
  ...
  _real w = (_real) parms.get_parameter('W')->get_float_value();
  ...
  *recon_dims = to_std_vector(matrix_size); recon_dims->push_back(frames_per_reconstruction); 
  ...
  std::cout << "RECON DIMS: ";
  for (int i = 0; i < recon_dims->size(); i++) std::cout << recon_dims->at(i) << " ";
  std::cout << std::endl;
  ...
  if( (alpha<1.0) && (lambda > 0)){
    ...
    sb.add_group(0);
  	/*
  	sb.add_regularization_operator( Rx ,1);
		sb.add_regularization_operator( Ry ,1);
		sb.add_regularization_operator( Rz,1 );
		*/
  }
  ...
  if( (alpha > 0.0) && (lambda > 0) ){
    ...
    sb.add_group(reg_image,0);

  	/*
  	sb.add_regularization_operator( Rx2 ,reg_image,1);
		sb.add_regularization_operator( Ry2 ,reg_image,1);
		sb.add_regularization_operator( Rz2,reg_image,1 );
		*/
  }
