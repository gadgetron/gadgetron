...
if [ $(id -u) -ne 0 ]; then
  ...
  if [ $# -eq 2 ]; then
    ...
    MAX_MOUNT_TRY=1000
    while [ ${MOUNT_READY} -eq 0 ]; do
      if mountpoint -q ${GLOBAL_MOUNT_POINT} && [ -e ${GLOBAL_MOUNT_POINT}/chroot-root/gadgetron/home/javeda2/anaconda3/envs/gadgetron/share/gadgetron/chroot/start.sh ]; then
          MOUNT_READY=1
      else
          sleep 0.2
          let MOUNT_TRY++
    ...
    if mountpoint -q ${GLOBAL_MOUNT_POINT}; then
        if ! mountpoint -q ${GLOBAL_MOUNT_POINT}/chroot-root/gadgetron/proc; then
            mount_safe "${GLOBAL_MOUNT_POINT}/chroot-root/gadgetron/proc" /proc self/exe
        fi

        if ! mountpoint -q ${GLOBAL_MOUNT_POINT}/chroot-root/gadgetron/dev; then
            mount_safe "${GLOBAL_MOUNT_POINT}/chroot-root/gadgetron/dev" /dev null
        fi

        if ! mountpoint -q ${GLOBAL_MOUNT_POINT}/chroot-root/gadgetron/sys; then
